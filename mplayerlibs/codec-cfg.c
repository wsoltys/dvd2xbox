...
#warning guid.Data1 lehet 0? honnan lehet tudni, hogy nem adtak meg?
...
loop_enter:
        ...
        else if (!strcmp(token[0], "guid"))
        {
            if (get_token(11, 11) < 0)
                goto err_out_parse_error;
            codec->guid.Data1=strtoul(token[0],&endptr,0);
            if ((*endptr != ',' || *(endptr + 1) != '\0') &&
                    *endptr != '\0')
                goto err_out_parse_error;
            codec->guid.Data2=strtoul(token[1],&endptr,0);
            if ((*endptr != ',' || *(endptr + 1) != '\0') &&
                    *endptr != '\0')
                goto err_out_parse_error;
            codec->guid.Data3=strtoul(token[2],&endptr,0);
            if ((*endptr != ',' || *(endptr + 1) != '\0') &&
                    *endptr != '\0')
                goto err_out_parse_error;
            for (i = 0; i < 8; i++)
            {
                codec->guid.Data4[i]=strtoul(token[i + 3],&endptr,0);
                if ((*endptr != ',' || *(endptr + 1) != '\0') &&
                        *endptr != '\0')
                    goto err_out_parse_error;
            }
        ...
        else if (!strcmp(token[0], "status"))
        {
            if (get_token(1, 1) < 0)
                goto err_out_parse_error;
            if (!strcmp(token[0], "working"))
                codec->status = CODECS_STATUS_WORKING;
            else if (!strcmp(token[0], "crashing"))
                codec->status = CODECS_STATUS_NOT_WORKING;
            else if (!strcmp(token[0], "untested"))
                codec->status = CODECS_STATUS_UNTESTED;
            else if (!strcmp(token[0], "buggy"))
...
void list_codecs(int audioflag)
{
    ...
    for (/* NOTHING */; i--; c++)
    {
        ...
        switch(c->status)
        {
        case CODECS_STATUS_WORKING:
            s="working ";
            break;
        case CODECS_STATUS_PROBLEMS:
            s="problems";
            break;
        case CODECS_STATUS_NOT_WORKING:
            s="crashing";
            break;
        case CODECS_STATUS_UNTESTED:
            s="untested";
            break;
        }
        if(c->dll)
...
void parsehtml(FILE *f1,FILE *f2,codecs_t *codec,int section,int dshow)
{
    ...
    while((c=fgetc(f1))>=0)
    {
        ...
        case 'n':
            wrapline(f2,codec->name);
            break;
        case 'i':
            wrapline(f2,codec->info);
            break;
        case 'c':
            wrapline(f2,codec->comment);
            break;
        case 'd':
            wrapline(f2,codec->dll);
            break;
        case 'D':
            fprintf(f2,"%c",codec->driver==dshow?'+':'-');
            break;
        case 'F':
            ...
            for(d=0;d<CODECS_MAX_OUTFMT;d++)
                if(codec->outfmt[d]!=0xFFFFFFFF)
                {
                    for (c=0; fmt_table[c].name; c++)
                        if(fmt_table[c].num==codec->outfmt[d])
                            break;
                    if(fmt_table[c].name)
...
int main(void)
{
    ...
    f1=fopen("DOCS/codecs-in.html","rb");
    if(!f1)
        exit(1);
    f2=fopen("DOCS/codecs-status.html","wb");
    if(!f2)
        exit(1);
...
next:
    if (c)
    {
        ...
        for(i=0;i<nr_codecs;i++, c++)
        {
            ...
            printf("GUID: %08lX %04X %04X",c->guid.Data1,c->guid.Data2,c->guid.Data3);
            for(j=0;j<8;j++)
                printf(" %02X",c->guid.Data4[j]);
            printf("\n");
