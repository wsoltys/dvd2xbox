...
static int init(int rate,int channels,int format,int flags)
{
    ...
    result = waveOutOpen(&hWaveOut,WAVE_MAPPER,&wformat,(DWORD*)waveOutProc,0,CALLBACK_FUNCTION);
...
static int write_waveOutBuffer(unsigned char* data,int len)
{
    ...
    int len2=0;
    int x;
		if(full_buffers< 0 || full_buffers>BUFFER_COUNT)
		{
			assert(0);
		}
    ...
    while(len>0)
    {
        ...
        if (MMSYSERR_NOERROR != waveOutPrepareHeader(hWaveOut, current, sizeof(WAVEHDR)))
				{
					assert(0);
				}

        if (MMSYSERR_NOERROR!=waveOutWrite(hWaveOut, current, sizeof(WAVEHDR)))
				{
					assert(0);
				}
