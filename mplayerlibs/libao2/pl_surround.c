...
static int init()
{
    ...
    mp_msg(0,0, "pl_surround: init input rate=%d, channels=%d\n", ao_plugin_data.rate, ao_plugin_data.channels);
    if (ao_plugin_data.channels != 2)
    {
        mp_msg(0,0, "pl_surround: source audio must have 2 channels, using passthrough mode\n");
        pl_surround.passthrough = 1;
        return 1;
    }
    if (ao_plugin_data.format != AFMT_S16_NE)
    {
        mp_msg(0,0, "pl_surround: I'm dumb and can only handle AFMT_S16_LE audio format, using passthrough mode\n");
        pl_surround.passthrough = 1;
        return 1;
    }
    ...
    mp_msg(0,0, "pl_surround: %dmsec surround delay, rate %d - buffers are %d bytes each\n",
...
static void uninit()
{
    //  mp_msg(0,0, "pl_surround: uninit called!\n");
    if (pl_surround.passthrough)
        return;
...
static void reset()
{
    if (pl_surround.passthrough)
        return;
    //mp_msg(0,0, "pl_surround: reset called\n");
    pl_surround.delaybuf_pos = 0;
...
static int play()
{
    ...
    // mp_msg(0,0, "pl_surround: play %d bytes, %d samples\n", ao_plugin_data.len, samples);

    samples  = ao_plugin_data.len / sizeof(int16_t) / pl_surround.input_channels;
    out = pl_surround.databuf;
    in = (int16_t *)ao_plugin_data.data;
