                  ...
                  unsigned int flags, unsigned int outfmt)
{
    ...
    {
        mp_msg(0,0,"yv12 not supported by next filter/vo :(\n");
        return 0;
    }
...
{
    if(cbp&1)
        drawBasis(dst[0]              , stride[0], amp, 1, dc);
    if(cbp&2)
        drawBasis(dst[0]+8            , stride[0], amp, 1, dc);
    if(cbp&4)
        drawBasis(dst[0]+  8*stride[0], stride[0], amp, 1, dc);
    if(cbp&8)
        drawBasis(dst[0]+8+8*stride[0], stride[0], amp, 1, dc);
    if(cbp&16)
        drawBasis(dst[1]              , stride[1], amp, 1, dc);
    if(cbp&32)
        drawBasis(dst[2]              , stride[2], amp, 1, dc);
}
...
static int put_image(struct vf_instance_s* vf, mp_image_t *mpi)
{
    ...
    if(frame%30)
    {
        ...
        {
        case 0:
            dc1Test(dmpi->planes[0], dmpi->stride[0], 256, 256, frame%30);
            break;
        case 1:
            dc1Test(dmpi->planes[1], dmpi->stride[1], 256, 256, frame%30);
            break;
        case 2:
            freq1Test(dmpi->planes[0], dmpi->stride[0], frame%30);
            break;
        case 3:
            freq1Test(dmpi->planes[1], dmpi->stride[1], frame%30);
            break;
        case 4:
            amp1Test(dmpi->planes[0], dmpi->stride[0], frame%30);
            break;
        case 5:
            amp1Test(dmpi->planes[1], dmpi->stride[1], frame%30);
            break;
        case 6:
            cbp1Test(dmpi->planes   , dmpi->stride   , frame%30);
            break;
        case 7:
            mv1Test(dmpi->planes[0], dmpi->stride[0], frame%30);
            break;
        case 8:
            ring1Test(dmpi->planes[0], dmpi->stride[0], frame%30);
            break;
        case 9:
            ring2Test(dmpi->planes[0], dmpi->stride[0], frame%30);
            break;
        }
    }
