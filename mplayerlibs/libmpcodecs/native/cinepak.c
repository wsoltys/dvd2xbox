...
static void read_codebook_yv12(cvid_codebook *c, int mode)
{
...
void cvid_v1_yv12(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb)
{
    unsigned char *p;
    int stride;

    if(y+3>=(unsigned int)mpi->height)
        return; // avoid sig11
...
/* ---------------------------------------------------------------------- */
void cvid_v4_yv12(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb0,
...
static void read_codebook_yuy2(cvid_codebook *c, int mode)
{
    unsigned char y0, y1, y2, y3, u, v;
    int y_uv;
...
/* ------------------------------------------------------------------------ */
void cvid_v1_yuy2(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb)
{
...
/* ------------------------------------------------------------------------ */
void cvid_v4_yuy2(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb0,
                  cvid_codebook *cb1, cvid_codebook *cb2, cvid_codebook *cb3)
{
    int stride = mpi->stride[0] / 2;
    unsigned long *vptr = (unsigned long *)mpi->planes[0];
...
/* ---------------------------------------------------------------------- */
static void read_codebook_32(cvid_codebook *c, int mode)
{
    int uvr, uvg, uvb;
...
/* ------------------------------------------------------------------------ */
void cvid_v1_32(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb)
{
...
/* ------------------------------------------------------------------------ */
void cvid_v4_32(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb0,
                cvid_codebook *cb1, cvid_codebook *cb2, cvid_codebook *cb3)
{
    int stride = mpi->stride[0]/4;
    unsigned long *vptr = (unsigned long *)mpi->planes[0];
...
/* ---------------------------------------------------------------------- */
static void read_codebook_24(cvid_codebook *c, int mode)
{
    int uvr, uvg, uvb;
...
/* ------------------------------------------------------------------------ */
void cvid_v1_24(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb)
{
    unsigned char r, g, b;
...
/* ------------------------------------------------------------------------ */
void cvid_v4_24(mp_image_t *mpi, unsigned int x, unsigned int y, cvid_codebook *cb0,
                cvid_codebook *cb1, cvid_codebook *cb2, cvid_codebook *cb3)
{
    int stride = (mpi->stride[0])-4*3;
    unsigned char *vptr = mpi->planes[0] + (y * mpi->stride[0]) + x * 3;
...
void *decode_cinepak_init(void)
{
    ...
    if((cvinfo = calloc(sizeof(cinepak_info), 1)) == NULL)
        return NULL;
    cvinfo->strip_num = 0;
...
void decode_cinepak(void *context, unsigned char *buf, int size, mp_image_t *mpi)
{
    ...
    {
        if(len & 0x01)
            len++; /* AVIs tend to have a size mismatch */
        if(len != size)
        {
