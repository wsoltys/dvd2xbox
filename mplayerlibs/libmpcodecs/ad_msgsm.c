...
static int init(sh_audio_t *sh_audio)
{
    ...
    {
        sh_audio->ds->ss_mul=65;
        sh_audio->ds->ss_div=320;
    }
    else
    {
        sh_audio->ds->ss_mul=33;
        sh_audio->ds->ss_div=160;
    }
...
static int decode_audio(sh_audio_t *sh_audio,unsigned char *buf,int minlen,int maxlen)
{
    if(sh_audio->format==0x31 || sh_audio->format==0x32)
    {
        ...
        if(demux_read_data(sh_audio->ds,ibuf,65)!=65)
            return -1; // EOF
    ...
    else
    {
        ...
        if(demux_read_data(sh_audio->ds,ibuf,33)!=33)
            return -1; // EOF
