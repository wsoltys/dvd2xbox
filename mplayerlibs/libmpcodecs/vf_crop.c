                  ...
                  unsigned int flags, unsigned int outfmt)
{
    ...
    if(vf->priv->crop_w<=0 || vf->priv->crop_w>width)
        vf->priv->crop_w=width;
    if(vf->priv->crop_h<=0 || vf->priv->crop_h>height)
        vf->priv->crop_h=height;
    if(vf->priv->crop_x<0)
        vf->priv->crop_x=(width-vf->priv->crop_w)/2;
    if(vf->priv->crop_y<0)
        vf->priv->crop_y=(height-vf->priv->crop_h)/2;
    // rounding:
    ...
    {
        mp_msg(0,0,"crop: bad position/width/height - cropped area is out of the original!\n");
        return 0;
    }
                       ...
                       unsigned char** src, int* stride, int w,int h, int x, int y)
{
    ...
    if (x+w > vf->priv->crop_w)
        w = vf->priv->crop_w-x;
    if (y+h > vf->priv->crop_h)
        h = vf->priv->crop_h-y;
