...
static __inline void *my_memcpy_pic(unsigned char * dst, unsigned char * src, int bytesPerLine, int height, int dstStride, int srcStride)
{
    int i;
    void *retval=dst;
...
static __inline int clamp(int a)
{
...
static int open(vf_instance_t *vf, char* args)
{
    ...
    if(gCpuCaps.hasMMX)
        qpel_li = qpel_li_MMX;
    if(gCpuCaps.hasMMX)
        qpel_4tap = qpel_4tap_MMX;
#endif
#ifdef HAVE_MMX2
    if(gCpuCaps.hasMMX2)
        qpel_li = qpel_li_MMX2;
#endif
#ifdef HAVE_3DNOW
    if(gCpuCaps.has3DNow)
        qpel_li = qpel_li_3DNOW;
#endif
    return 1;
}
