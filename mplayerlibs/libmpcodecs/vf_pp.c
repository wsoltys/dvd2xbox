                  ...
                  unsigned int voflags, unsigned int outfmt)
{
    ...
    if(vf->priv->context)
        pp_free_context(vf->priv->context);
...
{
    if(vf->priv->pp&0xFFFF)
        return; // non-local filters enabled
