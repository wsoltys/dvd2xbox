...
#include "../libdll/dshow/DS_AudioDecoder.h"
...
extern BOOL __stdcall FreeLibrary(HINSTANCE hLibModule);
extern HMODULE __stdcall LoadLibraryA(LPCSTR libname);
extern FARPROC __stdcall GetProcAddress( HMODULE hModule, LPCSTR function );

static unsigned long (__stdcall *wrvyuv_custom_message)(cmsg_data_t* ,void*);
static unsigned long (__stdcall *wrvyuv_free)(void*);
static unsigned long (__stdcall *wrvyuv_hive_message)(unsigned long,unsigned long);
static unsigned long (__stdcall *wrvyuv_init)(void*, void*); // initdata,context
static unsigned long (__stdcall *wrvyuv_transform)(char*, char*,transform_in_t*,unsigned int*,void*);
#endif
...
static void uninit(sh_video_t *sh)
{
    ...
    {
        if (wrvyuv_free)
            wrvyuv_free(sh->context);
    }
    else
#endif
        if(rvyuv_free)
            rvyuv_free(sh->context);
...
typedef struct dp_hdr_s
{
    ...
    unsigned int timestamp; // timestamp from packet header
    unsigned int len;	// length of actual data
    unsigned int chunktab;	// offset to chunk offset array
}
dp_hdr_t;
...
static mp_image_t* decode(sh_video_t *sh,void* data,int len,int flags)
{
    ...
    if(len<=0 || flags&2)
        return NULL; // skipped frame || hardframedrop
    ...
    {  // rv30 width/height now known
        ...
        if (!mpcodecs_config_vo(sh,sh->disp_w,sh->disp_h,IMGFMT_I420))
            return 0;
        mpi=mpcodecs_get_image(sh, MP_IMGTYPE_TEMP, 0 /*MP_IMGFLAG_ACCEPT_STRIDE*/,
                               sh->disp_w, sh->disp_h);
        if(!mpi)
