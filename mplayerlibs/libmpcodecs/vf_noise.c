...
#ifdef HAVE_MMX2
static  void lineNoise_MMX2(uint8_t *dst, uint8_t *src, int8_t *noise, int len, int shift)
{
    int mmx_len= len&(~7);
    noise+=shift;
...
static void uninit(struct vf_instance_s* vf)
{
    ...
    if(vf->priv->chromaParam.noise)
        free(vf->priv->chromaParam.noise);
    vf->priv->chromaParam.noise= NULL;

    if(vf->priv->lumaParam.noise)
        free(vf->priv->lumaParam.noise);
...
static int open(vf_instance_t *vf, char* args)
{
    ...
    if(args)
    {
        ...
        if(arg2)
            parse(&vf->priv->chromaParam, arg2+1);
