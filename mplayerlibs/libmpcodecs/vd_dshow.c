...
#define XTL_INCLUDED
#include "../libdll/dshow/DS_VideoDecoder.h"
...
extern FARPROC GetProcAddress(HINSTANCE hModule,LPCSTR lpProcName);
extern HINSTANCE LoadLibraryA(LPCSTR lpLibFileName);
extern BOOL FreeLibrary(HINSTANCE hLibModule);
...
static int init(sh_video_t *sh)
{
    ...
    case IMGFMT_YUY2:
    case IMGFMT_UYVY:
        DS_VideoDecoder_SetDestFmt(sh->context,16,out_fmt);
        break; // packed YUV
    case IMGFMT_YV12:
    case IMGFMT_I420:
    case IMGFMT_IYUV:
        DS_VideoDecoder_SetDestFmt(sh->context,12,out_fmt);
        break; // planar YUV
    case IMGFMT_YVU9:
        DS_VideoDecoder_SetDestFmt(sh->context,9,out_fmt);
        break;
    default:
