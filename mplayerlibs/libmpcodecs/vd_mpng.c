...
static void pngReadFN( png_structp pngstr,png_bytep buffer,png_size_t size )
{
    ...
    if(size>pngLength-pngPointer && pngLength>=pngPointer)
        size=pngLength-pngPointer;
...
static mp_image_t* decode(sh_video_t *sh,void* data,int len,int flags)
{
    ...
    if(last_w!=png_width || last_h!=png_height || last_c!=out_fmt)
    {
        ...
        if(!out_fmt)
            return NULL;
        if(!mpcodecs_config_vo(sh,png_width,png_height,out_fmt))
            return NULL;
    }
    ...
    for ( i=0; i < png_height; i++ )
        row_p[i]=mpi->planes[0] + mpi->stride[0]*i;
