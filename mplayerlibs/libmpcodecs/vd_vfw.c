...
static int init(sh_video_t *sh)
{
    ...
    if(ret < 0)
    {
        ...
        for (i=0; i < o_bih_len; i++)
            mp_msg(MSGT_WIN32, MSGL_DBG2, "%02x ", priv->o_bih[i]);
        return 0;
    }
    ...
    if(!sh_video->o_bih.biWidth)
        sh_video->o_bih.biWidth=sh_video->bih->biWidth;
    if(!sh_video->o_bih.biHeight)
        sh_video->o_bih.biHeight=sh_video->bih->biHeight;
    if(!sh_video->o_bih.biPlanes)
        sh_video->o_bih.biPlanes=sh_video->bih->biPlanes;
#endif
...
ret = ICDecompressBegin(priv->handle, sh->bih, priv->o_bih);
    ...
    if(verbose>0)
        print_video_header(sh->bih);
    mp_msg(MSGT_WIN32, MSGL_V, "Output format:\n");
    if(verbose>0)
        print_video_header(priv->o_bih);
