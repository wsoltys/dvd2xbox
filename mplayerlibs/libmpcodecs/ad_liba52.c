...
//static sample_t * a52samples;
static a52_state_t* a52_state=NULL;
static uint32_t a52_flags=0;

#include "../bswap.h"
extern uint16_t crc16_block(uint8_t *data,uint32_t num_bytes);
extern void* a52_resample_init(uint32_t mm_accel,int flags,int chans);
...
static int init(sh_audio_t *sh_audio)
{
    ...
    /* Dolby AC3 audio:*/
    if(gCpuCaps.hasMMX)
        a52_accel|=MM_ACCEL_X86_MMX;
    if(gCpuCaps.hasMMX2)
        a52_accel|=MM_ACCEL_X86_MMXEXT;
    if(gCpuCaps.has3DNow)
        a52_accel|=MM_ACCEL_X86_3DNOW;
...
{
	if ( a52_state) a52_free(a52_state);
	a52_state=NULL;
}
...
static int decode_audio(sh_audio_t *sh_audio,unsigned char *buf,int minlen,int maxlen)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        {
            mp_msg(MSGT_DECAUDIO,MSGL_ERR,"a52: error at resampling\n");
            break;
        }
				{
					sample_t *pSample=a52_samples (a52_state);
					len+=2*a52_resample(pSample,(int16_t *)&buf[len]);
				}
    }
    return len;
}
