...
#include "../libdll/dmo/DMO_AudioDecoder.h"
...
static int preinit(sh_audio_t *sh_audio)
{
    ...
    if(sh_audio->audio_in_minsize<8192)
        sh_audio->audio_in_minsize=8192;
...
static int decode_audio(sh_audio_t *sh_audio,unsigned char *buf,int minlen,int maxlen)
{
    ...
    if(srcsize>sh_audio->a_in_buffer_size)
        srcsize=sh_audio->a_in_buffer_size; // !!!!!!
    ...
    //mp_msg(MSGT_DECAUDIO,MSGL_DBG2,"DMO: audio %d -> %d converted  (in_buf_len=%d of %d)  %d\n",size_in,size_out,sh_audio->a_in_buffer_len,sh_audio->a_in_buffer_size,ds_tell_pts(sh_audio->ds));
