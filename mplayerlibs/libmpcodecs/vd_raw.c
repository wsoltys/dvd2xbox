...
static int control(sh_video_t *sh,int cmd,void* arg,...)
{
    ...
    case VDCTRL_QUERY_FORMAT:
        if( (*((int*)arg)) == (sh->bih ? sh->bih->biCompression : sh->format) )
            return CONTROL_TRUE;
...
static int init(sh_video_t *sh)
{
    ...
    if(sh->bih && sh->bih->biCompression==0)
    {	// set based on bit depth
        switch(sh->bih->biBitCount)
        {
        case 1:
            sh->bih->biCompression=IMGFMT_BGR1;
            break;
        case 4:
            sh->bih->biCompression=IMGFMT_BGR4;
            break;
        case 8:
            sh->bih->biCompression=IMGFMT_BGR8;
            break;
        case 15:
            sh->bih->biCompression=IMGFMT_BGR15;
            break;
        ...
        case 16:
            ...
            break;
        case 24:
            sh->bih->biCompression=IMGFMT_BGR24;
            break;
        case 32:
            sh->bih->biCompression=IMGFMT_BGR32;
            break;
        default:
...
static mp_image_t* decode(sh_video_t *sh,void* data,int len,int flags)
{
    ...
    else
    {
        ...
        if(sh->format==0 || sh->format==3)
            mpi->stride[0]=(mpi->stride[0]+3)&(~3);
        ...
        frame_size=mpi->stride[0]*mpi->h;
        if(mpi->bpp<8)
            frame_size=frame_size*mpi->bpp/8;
    }
