...
static void remove_func(int x0,int y0, int w,int h)
{
    ...
    if(y0+h>vf->priv->exp_y+orig_h)
    {
        ...
        int y=y0;
        if(y<vf->priv->exp_y+orig_h)
            y=vf->priv->exp_y+orig_h;
        remove_func_2(x0,y,w,y0+h-y);
        if(y0>=vf->priv->exp_y+orig_h)
            return;
        h=y-y0;
    }
    if(x0>=vf->priv->exp_x || x0+w<=vf->priv->exp_x+orig_w)
        return;
...
#if 0
    if(vf->priv->exp_w<width)
        vf->priv->exp_w=width;
    if(vf->priv->exp_h<height)
        vf->priv->exp_h=height;
#else
if ( vf->priv->exp_w == -1 )
    vf->priv->exp_w=width;
    else if (vf->priv->exp_w < -1 )
    ...
    if ( vf->priv->exp_h == -1 )
        ...
        vf->priv->exp_h=height;
#endif
    if(vf->priv->exp_x<0 || vf->priv->exp_x+width>vf->priv->exp_w)
        vf->priv->exp_x=(vf->priv->exp_w-width)/2;
    if(vf->priv->exp_y<0 || vf->priv->exp_y+height>vf->priv->exp_h)
...
static void get_image(struct vf_instance_s* vf, mp_image_t *mpi)
{
    ...
    if(vf->priv->osd && (mpi->flags&MP_IMGFLAG_PRESERVE))
    {
        ...
        if(vo_osd_check_range_update(vf->priv->exp_x,vf->priv->exp_y,
                                     vf->priv->exp_x+mpi->w,vf->priv->exp_y+mpi->h))
            return;
    }
#endif
