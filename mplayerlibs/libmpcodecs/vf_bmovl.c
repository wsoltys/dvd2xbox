...
put_image(struct vf_instance_s* vf, mp_image_t* mpi)
{
    ...
    if(vf->priv->stream_fd >= 0)
    {
        ...
        if( select( vf->priv->stream_fd+1, &vf->priv->stream_fdset, NULL, NULL, &tv ) > 0)
        {
            ...
            command=NONE;
            if     ( strncmp(cmd,"RGBA32",6)==0 )
            {
                pxsz=4;
                command = IMG_RGBA32;
            }
            else if( strncmp(cmd,"ABGR32",6)==0 )
            {
                pxsz=4;
                command = IMG_ABGR32;
            }
            else if( strncmp(cmd,"RGB24" ,5)==0 )
            {
                pxsz=3;
                command = IMG_RGB24;
            }
            else if( strncmp(cmd,"BGR24" ,5)==0 )
            {
                pxsz=3;
                command = IMG_BGR24;
            }
    ...
    { // Blit the bitmap to the videoscreen, pixel for pixel
        for( ypos=vf->priv->y1 ; ypos < vf->priv->y2 ; ypos++ )
        {
            for ( xpos=vf->priv->x1 ; xpos < vf->priv->x2 ; xpos++ )
            {
                ...
                if (alpha == 0)
                    continue; // Completly transparent pixel
