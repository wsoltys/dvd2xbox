...
int init_video(sh_video_t *sh_video,char* codecname,char* vfm,int status)
{
    ...
    while(1)
    {
        ...
        for (i=0; mpcodecs_vd_drivers[i] != NULL; i++)
            ...
            if(!strcmp(mpcodecs_vd_drivers[i]->info->short_name,sh_video->codec->drv))
                break;
...
int init_best_video_codec(sh_video_t *sh_video,char** video_codec_list,char** video_fm_list)
{
            ...
            if(video_fm_list)
            {
                ...
                while(!sh_video->inited && *fmlist)
                {
                    ...
                    for(status=CODECS_STATUS__MAX;status>=CODECS_STATUS__MIN;--status)
                        if(init_video(sh_video,NULL,video_fm,status))
                            break;
                }
            }
            if(!sh_video->inited)
                for(status=CODECS_STATUS__MAX;status>=CODECS_STATUS__MIN;--status)
                    if(init_video(sh_video,NULL,NULL,status))
                        break;
        }
    }
    ...
    if(!sh_video->inited)
    {
        mp_msg(MSGT_DECVIDEO,MSGL_HINT, MSGTR_TryUpgradeCodecsConfOrRTFM,"codecs.conf");
