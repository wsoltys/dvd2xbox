...
void vf_mpi_clear(mp_image_t* mpi,int x0,int y0,int w,int h)
{
    ...
    for(y=y0;y<y0+h;y++)
    {
        ...
        if(mpi->flags&MP_IMGFLAG_YUV)
        {
            ...
            {
                for(i=0;i<size;i+=4)
                    p[i]=p[i+1]=p[i+2]=p[i+3]=0x00800080;
                for(;i<size;i++)
                    p[i]=0x00800080;
            }
            else
            {
                for(i=0;i<size;i+=4)
                    p[i]=p[i+1]=p[i+2]=p[i+3]=0x80008000;
                for(;i<size;i++)
                    p[i]=0x80008000;
            }
        }
    ...
    if(mpi)
    {
        ...
        if(mpi->width!=w2 || mpi->height!=h)
        {
            ...
            else
            {
                ...
                mpi->height=h;
                mpi->chroma_height=(h + (1<<mpi->chroma_y_shift) - 1)>>mpi->chroma_y_shift;
            }
        }
        if(!mpi->bpp)
            ...
            if(!(mpi->flags&MP_IMGFLAG_DIRECT))
            {
                ...
                if(mp_imgflag&MP_IMGFLAG_PREFER_ALIGNED_STRIDE)
                {
                    ...
                    if(mpi->width!=w2)
                    {
                        ...
                        if(!(flags&3))
                            mp_msg(MSGT_DECVIDEO,MSGL_WARN,"??? vf_get_image{vf->query_format(outfmt)} failed!\n");
                        //		  printf("query -> 0x%X    \n",flags);
...
vf_instance_t* vf_open_plugin(vf_info_t** filter_list, vf_instance_t* next, char *name, char **args)
{
    ...
    { // vf_vo get some special argument
        ...
        if(args && !strcmp(args[0],"_oldargs_"))
            ...
            args = NULL;
    if(vf->info->open(vf,(char*)args)>0)
        return vf; // Success!
    free(vf);
