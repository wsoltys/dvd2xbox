...
#error we dont support libavcodec prior to build 4641, get the latest libavcodec CVS
	...
	#error your version of libavcodec is old, u might want to get a newer one 
#endif 
...
static void uninit(sh_video_t *sh){
    ...
    if(lavc_param_vstats){
			 mp_msg(0,0,"Arithmetic mean of QP: %2.4f, Harmonic mean of QP: %2.4f\n",  
...
static int init_vo(sh_video_t *sh)
{
            ...
            !ctx->vo_inited)
    {
        ...
        case PIX_FMT_YUV410P:
            ctx->best_csp=IMGFMT_YVU9;
            break; //svq1
        case PIX_FMT_YUV420P:
            ctx->best_csp=IMGFMT_YV12;
            break; //mpegs
        case PIX_FMT_YUV422P:
            ctx->best_csp=IMGFMT_422P;
            break; //mjpeg / huffyuv
        case PIX_FMT_YUV444P:
            ctx->best_csp=IMGFMT_444P;
            break; //photo jpeg
        case PIX_FMT_YUV411P:
            ctx->best_csp=IMGFMT_411P;
            break; //dv ntsc
        case PIX_FMT_YUV422:
            ctx->best_csp=IMGFMT_YUY2;
            break; //huffyuv perhaps in the future
        case PIX_FMT_RGB24 :
            ctx->best_csp=IMGFMT_BGR24;
            break; //huffyuv
        case PIX_FMT_RGBA32:
            ctx->best_csp=IMGFMT_BGR32;
            break; //huffyuv
        default:
            ctx->best_csp=0;
        }
...
// decode a frame
static mp_image_t* decode(sh_video_t *sh,unsigned char* data,int len,int flags)
{
    int got_picture=0;
    int ret;
    vd_ffmpeg_ctx *ctx = sh->context;
...
/*
    while(lavc_param_vstats)
    { // always one time loop
        static FILE *fvstats=NULL;
        char filename[20];
        static __int64 all_len=0;
