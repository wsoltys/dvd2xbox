...
#define XTL_INCLUDED
#include "../libdll/dmo/DMO_VideoDecoder.h"
...
static int init(sh_video_t *sh)
{
    ...
    case IMGFMT_YUY2:
    case IMGFMT_UYVY:
        DMO_VideoDecoder_SetDestFmt(sh->context,16,out_fmt);
        break; // packed YUV
    case IMGFMT_YV12:
    case IMGFMT_I420:
    case IMGFMT_IYUV:
        DMO_VideoDecoder_SetDestFmt(sh->context,12,out_fmt);
        break; // planar YUV
    case IMGFMT_YVU9:
        DMO_VideoDecoder_SetDestFmt(sh->context,9,out_fmt);
        break;
    default:
