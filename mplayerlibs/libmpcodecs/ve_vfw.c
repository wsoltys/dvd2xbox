...
static BITMAPINFOHEADER* vfw_open_encoder(char *dll_name, BITMAPINFOHEADER *input_bih,unsigned int out_fourcc)
{
...
#if 1
    {
        ...
        mp_msg(0,0,"Compressor type: %.4lx\n", icinfo.fccType);
        mp_msg(0,0,"Compressor subtype: %.4lx\n", icinfo.fccHandler);
        mp_msg(0,0,"Compressor flags: %lu, version %lu, ICM version: %lu\n",
               icinfo.dwFlags, icinfo.dwVersion, icinfo.dwVersionICM);
        //mp_msg(0,0,"Compressor name: %s\n", icinfo.szName);
        //mp_msg(0,0,"Compressor description: %s\n", icinfo.szDescription);

        mp_msg(0,0,"Flags:");
        if (icinfo.dwFlags & VIDCF_QUALITY)
            mp_msg(0,0," quality");
        if (icinfo.dwFlags & VIDCF_FASTTEMPORALD)
            mp_msg(0,0," fast-decompr");
        if (icinfo.dwFlags & VIDCF_QUALITYTIME)
            mp_msg(0,0," temp-quality");
        mp_msg(0,0,"\n");
    }
#endif

    temp_len = ICCompressGetFormatSize(encoder_hic, input_bih);
    mp_msg(0,0,"ICCompressGetFormatSize ret: %d\n", temp_len);
...
static int vfw_start_encoder(BITMAPINFOHEADER *input_bih, BITMAPINFOHEADER *output_bih)
{
    ...
    if(ret < 0)
    {
        ...
        for (i=0; i < temp_len; i++)
            mp_msg(MSGT_WIN32, MSGL_DBG2, "%02x ", temp[i]);
        return 0;
    }
    ...
    mp_msg(0,0," Output format after query/begin:\n");
    mp_msg(0,0,"  biSize %d\n", output_bih->biSize);
    mp_msg(0,0,"  biWidth %d\n", output_bih->biWidth);
    mp_msg(0,0,"  biHeight %d\n", output_bih->biHeight);
    mp_msg(0,0,"  biPlanes %d\n", output_bih->biPlanes);
                  ...
                  unsigned int flags, unsigned int outfmt)
{
    ...
    if(!vfw_start_encoder(vfw_bih, mux_v->bih))
        return 0;
...
static int vf_open(vf_instance_t *vf, char* args)
{
    ...
    if (!vfw_param_codec)
    {
        mp_msg(0,0,"No VfW codec specified! It's required!\n");
        return 0;
    }
