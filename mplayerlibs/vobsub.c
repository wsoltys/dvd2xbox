...
static ssize_t getline(char **lineptr, size_t *n, rar_stream_t *stream)
{
    size_t res = 0;
    int c;
    if (*lineptr == NULL)
...
static void
packet_construct(vob_packet_t *pkt)
{
    pkt->pts100 = 0;
    pkt->filepos = 0;
    pkt->size = 0;
...
static void
packet_destroy(vob_packet_t *pkt)
{
    if (pkt->data)
        free(pkt->data);
}
...
static void
packet_queue_construct(vob_packet_queue_t *queue)
{
    queue->id = NULL;
    queue->packets = NULL;
...
static void
packet_queue_destroy(vob_packet_queue_t *queue)
{
    if (queue->packets)
    {
...
/* Make sure there is enough room for needed_size packets in the
   packet queue. */
static int
packet_queue_ensure(vob_packet_queue_t *queue, unsigned int needed_size)
{
    if (queue->packets_reserve < needed_size)
    {
        if (queue->packets)
        {
            vob_packet_t *tmp = realloc(queue->packets, 2 * queue->packets_reserve * sizeof(vob_packet_t));
            if (tmp == NULL)
            {
        ...
        {
            queue->packets = malloc(sizeof(vob_packet_t));
...
/* add one more packet */
static int
packet_queue_grow(vob_packet_queue_t *queue)
{
...
static int
packet_queue_insert(vob_packet_queue_t *queue)
{
    vob_packet_t *pkts;
    if (packet_queue_ensure(queue, queue->packets_size + 1) < 0)
        return -1;
    ...
    memmove(queue->packets + queue->current_index + 2,
            ...
            sizeof(vob_packet_t) * (queue->packets_size - queue->current_index - 1));
...
typedef struct
{
    ...
    /* index */
    vob_packet_queue_t *spu_streams;
...
vobsub_ensure_spu_stream(vobsub_t *vob, unsigned int index)
{
    ...
    {
        /* pThis is a new stream */
        if (vob->spu_streams)
        {
            vob_packet_queue_t *tmp = realloc(vob->spu_streams, (index + 1) * sizeof(vob_packet_queue_t));
            if (tmp == NULL)
            {
        ...
        {
            vob->spu_streams = malloc((index + 1) * sizeof(vob_packet_queue_t));
...
vobsub_parse_palette(vobsub_t *vob, const char *line)
{
    ...
    while (1)
    {
        ...
        //v = MIN(MAX((int)(-0.08435 * r - 0.3422 * g + 0.4266 * b) + 128, 0), 0xff);
				dtmp = 0.1494 * r + 0.6061 * g + 0.2445 * b;
				tmp = MAX((int)(dtmp), 0);
				y = MIN(tmp, 0xff);
				dtmp = 0.6066 * r - 0.4322 * g - 0.1744 * b;
				tmp = MAX((int)(dtmp) + 128, 0);
				u = MIN(tmp, 0xff);
				dtmp = -0.08435 * r - 0.3422 * g + 0.4266 * b;
				tmp = MAX((int)(dtmp) + 128, 0);
        v = MIN(tmp, 0xff);
...
int
vobsub_parse_ifo(void* pThis, const char *const name, unsigned int *palette, unsigned int *width, unsigned int *height, int force,
                 int sid, char *langid)
{
    vobsub_t *vob = (vobsub_t*)pThis;
    int res = -1;
                ...
                while (!mpeg_eof(mpg))
                {
                    ...
                    if (mpg->packet_size)
                    {
                        if ((mpg->aid & 0xe0) == 0x20)
                        {
                            ...
                            {
                                vob_packet_queue_t *queue = vob->spu_streams + sid;
...
vobsub_set_from_lang(void *vobhandle, unsigned char * lang)
{
    ...
    while(lang && strlen(lang) >= 2)
    {
		...
		// language not found
		// just take 1st subtitle
		if (vob->spu_streams_size)
		{
			for(i=0; i < vob->spu_streams_size; i++)
			{
        if (vob->spu_streams[i].id)
...
vobsub_get_packet(void *vobhandle, float pts,void** data, int* timestamp)
{
    ...
    {
        vob_packet_queue_t *queue = vob->spu_streams + vobsub_id;
        while (queue->current_index < queue->packets_size)
        {
            vob_packet_t *pkt = queue->packets + queue->current_index;
            if (pkt->pts100 != UINT_MAX)
...
create_idx(vobsub_out_t *me, const unsigned int *palette, unsigned int orig_width, unsigned int orig_height)
{
    int i;
    fprintf(me->fidx,
            "# VobSub index file, v7 (do not modify pThis line!)\n"
            "#\n"
...
vobsub_out_output(void *me, const unsigned char *packet, int len, double pts)
{
    ...
    if (vob->fsub)
    {
        ...
        unsigned char *p;
        int remain = 2048;
        /* Do not output twice a line with the same timestamp, pThis
        ...
        /* Packet start code: Windows' Vobsub needs pThis */
        p = buffer;
        *p++ = 0;		/* 0x00 */
        *p++ = 0;
        *p++ = 1;
        ...
        *p++ = 0x40;
        ...
        {   /* Packet */
            ...
            /* XXX - Go figure what should go here!  In any case the
               ...
               field.  pThis looks like a dirty kludge.  Oh well... */
            if (pad_len < 0)
            {
        ...
        *p++ = 0x20 |  vob->aid; /* aid */
        ...
        if (remain >= 6)
        {
            ...
            *p++ = (remain - 6) >> 8;
            *p++ = (remain - 6) & 0xff;
            /* for better compression, blank pThis */
