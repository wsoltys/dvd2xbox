...
static void vivo_parse_text_header(demuxer_t *demux, int header_len)
{
    ...
    i=0;
    while(i<header_len && buf[i]==0x0D && buf[i+1]==0x0A)
...
/* this is enought for check (for now) */
    ...
    // parse header:
    i=0;
    while(i<len && buf[i]==0x0D && buf[i+1]==0x0A)
        i+=2; // skip empty lines
    if(strncmp(buf+i,"Version:Vivo/",13))
        return 0; // bad version/type!
#endif
...
int demux_vivo_fill_buffer(demuxer_t *demux)
{
    ...
    if (c == 0x82)
    {
        ...
        prefix = 1;
        c = stream_read_char(demux->stream);
        mp_msg(0,0,"packet 0x82(pos=%u) chunk=%x\n",
    ...
    case 0x00: // header - skip it!
        {
            ...
            if(len>=0x80)
                len=0x80*(len-0x80)+stream_read_char(demux->stream);
            mp_msg(0,0,"vivo extra header: %d bytes\n",len);
#ifdef TEXTPARSE_ALL
            {
                int pos;
...
static int h263_decode_picture_header(unsigned char *b_ptr)
{
    ...
    if (get_bits1(&s->gb) != 1)
    {
        mp_msg(0,0,"bad marker\n");
        return -1;	/* marker */
    }
    if (get_bits1(&s->gb) != 0)
    {
        mp_msg(0,0,"bad h263 id\n");
        return -1;	/* h263 id */
    }
    ...
    if (format != 7)
    {
        mp_msg(0,0,"h263_plus = 0  format = %d\n",format);
        /* H.263v1 */
        ...
        mp_msg(0,0,"pict_type=%d\n",get_bits1(&s->gb));
        mp_msg(0,0,"unrestricted_mv=%d\n",get_bits1(&s->gb));
#if 1
        mp_msg(0,0,"SAC: %d\n",get_bits1(&s->gb));
        mp_msg(0,0,"advanced prediction mode: %d\n",get_bits1(&s->gb));
        mp_msg(0,0,"PB frame: %d\n",get_bits1(&s->gb));
#else
if (get_bits1(&s->gb) != 0)
        return -1;	/* SAC: off */
        if (get_bits1(&s->gb) != 0)
    ...
    {
        mp_msg(0,0,"h263_plus = 1\n");
        /* H.263v2 */
        if (get_bits(&s->gb, 3) != 1)
        {
            mp_msg(0,0,"H.263v2 A error\n");
            return -1;
        }
        if (get_bits(&s->gb, 3) != 6)
        { /* custom source format */
            mp_msg(0,0,"custom source format\n");
            return -1;
        }
        skip_bits(&s->gb, 12);
        skip_bits(&s->gb, 3);
...
void demux_open_vivo(demuxer_t* demuxer)
{
...

    {
        ...
        mp_msg(0,0,"VIVO Video stream %d size: display: %dx%d, codec: %ux%u\n",
    ...
    if (demuxer->audio->id >= -1)
    {
        ...
        else
        {
            ...
            {
                if (!strcmp(vivo_param_acodec, "g723"))
                    priv->audio_codec = VIVO_AUDIO_G723;
                if (!strcmp(vivo_param_acodec, "siren"))
                    priv->audio_codec = VIVO_AUDIO_SIREN;
            }
