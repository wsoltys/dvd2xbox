...
int demux_audio_open(demuxer_t* demuxer)
{
    ...
    case MP3:
        ...
        for(n = 0; n < 5 ; n++)
        {
            ...
            if(pos < 0)
                break;//return 0; // changed to play more mp3's
            stream_skip(s,pos-4);
            if(s->eof)
                return 0;
            stream_read(s,hdr,4);
        ...
        if(s->end_pos)
        {
            ...
            stream_read(s,tag,3);
						s->eof=0;// FIX EOF WHEN STREAMING
            tag[3] = '\0';
            if(strcmp(tag,"TAG"))
    ...
    case WAV:
        {
            ...
            if(l < 16)
            {
                mp_msg(0,0,"Bad wav header length : too short !!!\n");
...
int demux_audio_fill_buffer(demux_stream_t *ds)
{
    ...
    default:
        mp_msg(0,0,"Audio demuxer : unknown format %d\n",priv->frmt);
    }
...
int demux_audio_control(demuxer_t *demuxer,int cmd, void *arg)
{
    ...
    switch(cmd)
    {
			case DEMUXER_CTRL_GET_TIME_LENGTH:
					*((unsigned long *)arg)= (demuxer->movi_end-demuxer->movi_start)/(float)sh_audio->i_bps;
