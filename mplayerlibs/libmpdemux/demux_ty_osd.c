...
static int ty_CCdecode( char b1, char b2 )
{
    ...
    if ( b1 & 0x60 )                // text
    {
        if ( !TY_OSD_debug && TY_CC_stat == TY_CCNONE )
            return 0;
        if ( TY_OSD_debug > 3 )
        {
        ...
        if ( TY_CC_stat > 0 && TY_OSD_flags & TY_OSD_MODE )
            ty_DrawOSD();
    }
            ...
            case 0x05:                      // misc + F
                {
                    ...
                    case 0x25 ... 0x27:       // 2-4 row captions
                        {
                            if ( TY_CC_stat == TY_CCPOPUP )
                                ty_ClearOSD( 1 );
                            TY_CC_stat = b2 - 0x23;
                            if ( TY_CC_CUR_Y < TY_CC_stat )
                                TY_CC_CUR_Y = TY_CC_stat;
                            break;
                        }
            ...
            case 0x0301:                    // future
                {
                    ...
                    if ( TY_XDS_ptr[ 3 ] & 0x10 )
                        ty_AddXDSToDisplay( " TAPE" );
                }
                break;
