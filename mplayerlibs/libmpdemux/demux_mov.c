...
#include "../libmpcodecs/components.h"
#endif
    ...
    for (i=0; i < trak->chunks_size; i++)
    {
        ...
        if(!trak->samples_size)
        {
            ...
            if(trak->durmap_size==1 || (trak->durmap_size==2 && trak->durmap[1].num==1))
            {
                ...
                mp_msg(MSGT_DEMUX, MSGL_ERR, "*** constant samplesize & variable duration not yet supported! ***\nContact the author if you have such sample file!\n");
            return;
        }
        ...
        if(trak->editlist_size>0)
        {
            ...
            for(i=0;i<trak->editlist_size;i++)
            {
                ...
                {
                    if(pts<=trak->samples[sample].pts)
                        break;
                }
                el->start_sample=sample;
                el->pts_offset=((INT64)e_pts*(INT64)trak->timescale)/(INT64)timescale-trak->samples[sample].pts;
                pts+=((INT64)el->dur*(INT64)trak->timescale)/(INT64)timescale;
                e_pts+=el->dur;
                // find end sample
                for(;sample<trak->samples_size;sample++)
                {
...
//                       priv->mdat_start, priv->mdat_end);
                ...
                if(no==0)
        {
            ...
            if(!stream_skip(demuxer->stream,len-skipped))
                break;
            ++no;
        }
    ...
    static void lschunks(demuxer_t* demuxer,int level,off_t endpos,mov_track_t* trak)
    {
        ...
        //    mp_msg(0,0,"lschunks (level=%d,endpos=%x)\n", level, endpos);
        while(1)
        {
            off_t pos;
            off_t len;
...
#if 0
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword1: 0x%08X (%d)\n",tmp,tmp);
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword2: 0x%08X (%d)\n",tmp,tmp);
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword3: 0x%08X (%d)\n",tmp,tmp);
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword4: 0x%08X (%d)\n",tmp,tmp);
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword5: 0x%08X (%d)\n",tmp,tmp);
                        tmp=stream_read_dword(demuxer->stream);
                        mp_msg(0,0,"dword6: 0x%08X (%d)\n",tmp,tmp);
#endif
                ...
                case MOV_FOURCC('s','t','s','s'):
                    {
                        ...
                        //		for (i=0;i<entries;i++) mp_msg(0,0,"%3d: %d\n",i,trak->keyframes[i]);
                        break;
                    }
                ...
                case MOV_FOURCC('m','v','h','d'):
                    {
                        ...
                        //mp_msg(MSGT_DEMUX, MSGL_V,"MOV: %*sMovie header (%d bytes): tscale=%d  dur=%d\n",level,"",(int)len, (int)priv->timescale,(int)priv->duration);
                        break;
                    }
                case MOV_FOURCC('t','r','a','k'):
                    {
                        //	    if(trak) mp_msg(0,0,"MOV: Warning! trak in trak?\n");
                        ...
                        case MOV_TRAK_AUDIO:
                            {
                                ...
                                /*mp_msg(0,0,"MOV: timescale: %d samplerate: %d durmap: %d (%d) -> %d (%d)\n",
                                ...
                                if(trak->stdata_len >= 44 && trak->stdata[9]>=1)
                                {
                                    ...
                                    if(trak->stdata_len>=44+8)
                                    {
                                        ...
                                        //mp_msg(0,0,"Audio extra header: len=%d  fcc=0x%X\n",len,fcc);
                                        sh->codecdata_len = len-8;
                                        sh->codecdata = trak->stdata+44+8;
                                    }
                                }

                                if((trak->stdata[9]==0 || trak->stdata[9]==1) && trak->stdata_len >= 36)
                        ...
                        case MOV_TRAK_VIDEO:
                            {
                                ...
                                // palettized?
                                if (depth > 32)
                                    depth&=31; // depth > 32 means grayscale
                                ...
                                if (palette_count)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for (i = start; i <= end; i++)
                                        {
                                            ...
                                            {
                                                palette_map[entry
                                                            * 4 + 2] = trak->stdata[hdr_ptr + 0];
                                                palette_map[entry
                                                            * 4 + 1] = trak->stdata[hdr_ptr + 2];
                                                palette_map[entry
                                                            * 4 + 0] = trak->stdata[hdr_ptr + 4];
                        ...
                        case MOV_TRAK_GENERIC:
...
#if 0
                            {
                                ...
                                for (i=0; i<trak->samples_size; i++)
                                {
                                    ...
                                    _snprintf((char *)&name[0], 20, "samp%d", i);
                                ...
                                for (i=0; i<trak->chunks_size; i++)
                                {
                                    ...
                                    _snprintf((char *)&name[0], 20, "chunk%d", i);
                                    fd = open((char *)&name[0], O_CREAT|O_WRONLY);
                                ...
                                if (trak->samplesize > 0)
                                {
                                    ...
                                    _snprintf((char *)&name[0], 20, "trak%d", trak->id);
                                    fd = open((char *)&name[0], O_CREAT|O_WRONLY);
...
case MOV_FOURCC('m','o','o','v'):
            ...
            if(pos>=endpos)
                break;
            if(!stream_seek(demuxer->stream,pos))
                break;
        }
    }
    ...
    int mov_read_header(demuxer_t* demuxer)
    {
        ...
        if(demuxer->audio->id==-1 && best_a_id>=0)
            demuxer->audio->id=best_a_id;
        if(demuxer->video->id==-1 && best_v_id>=0)
            demuxer->video->id=best_v_id;
            ...
            for(t_no=0;t_no<priv->track_db;t_no++)
            {
                ...
                if(trak->type==MOV_TRAK_GENERIC)
                {
                    ...
                    for (i=0; i<trak->samples_size; i++)
                    {
                        ...
                        char buf[4096];
                        stream_seek(demuxer->stream, trak->samples[i].pos);
                        _snprintf(name, 20, "t%02d-s%03d.%s", t_no,i,
