...
//	END BOBBIN007
    ...
    //============ Open DVD title ==============
#ifdef USE_DVDNAV
    if(dvd_nav||strncmp("dvdnav://",filename,9) == 0)
    {
        dvdnav_priv_t *dvdnav_priv;
        int event,len,tmplen=0;
        char* name = NULL;//(filename[9] == '\0') ? NULL : filename + 9;
...
#endif
#ifdef USE_DVDREAD
    if( dvd_title||strncmp("dvd://",filename,6) == 0)
    {
        //  int ret,ret2;
        dvd_priv_t *d;
        int ttn,pgc_id,pgn;
        ...
        dvd_title = 1;//filename[6] == '\0' ? 1 : strtol(filename + 6,NULL,0);
        /**
         * Open the disc.
         */
        if(!dvd_device)
        ...
        // store data
        d=malloc(sizeof(dvd_priv_t));
        memset(d,0,sizeof(dvd_priv_t));
        d->dvd=dvd;
        d->title=title;
        d->vmg_file=vmg_file;
        d->tt_srpt=tt_srpt;
...
//mp_msg(MSGT_OPEN,MSGL_ERR,MSGTR_SMBNotCompiled,filename);
  //          return NULL;
    ...
    {
        filename += 6;
#if defined(__CYGWIN__) || defined(__MINGW32__) || defined(__XBOX)
...
navRead_DSI( &d->dsi_pack, &(data[ DSI_START_BYTE ]), sizeof(dsi_t) );
    ...
    {
        if(d->angle_seek==2)
            mp_msg(MSGT_DVD,MSGL_V, "!!! warning! reading packet while angle_seek !!!\n");
