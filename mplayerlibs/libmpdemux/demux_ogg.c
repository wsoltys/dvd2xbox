...
#else
#include "../libmpcodecs/vorbis/ogg.h"
#include "../libmpcodecs/vorbis/codec.h"
#endif

#ifdef HAVE_OGGTHEORA
#include "../libmpcodecs/theora.h"
#endif
...
void demux_ogg_add_sub (ogg_stream_t* os,ogg_packet* pack)
{
    ...
    ogg_sub.lines = 0;
    if (((unsigned char)packet[0]) == 0x88)
    { // some subtitle text
        // Find data start
        int32_t duration = 0;
        ...
        while (1)
        {
            ...
            if(c=='\n' || c==0 || line_pos >= OGG_SUB_MAX_LINE-1)
            {
                ...
                if(line_pos)
                    ogg_sub.lines++;
                if(!c || ogg_sub.lines>=SUB_MAX_TEXT)
                    break; // EOL or TooMany
                line_pos = 0;
            }
            switch (c)
            {
...
static int demux_ogg_add_packet(demux_stream_t* ds,ogg_stream_t* os,ogg_packet* pack)
{
    ...
    ds_add_packet(ds,dp);
    if(verbose>1)
        printf("New dp: %p  ds=%p  pts=%5.3f  len=%d  flag=%d  \n",
...
int demux_ogg_control(demuxer_t *demuxer,int cmd, void *arg)
{
    ...
    switch(cmd)
    {
			...
			{
				*((unsigned long *)arg)= (demuxer->movi_end-demuxer->movi_start)/(float)(sh_audio->i_bps*(sh_audio->channels/2));
			}
