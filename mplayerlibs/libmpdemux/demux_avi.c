...
if((unsigned long)((AVIINDEXENTRY *)priv->idx)[0].dwChunkOffset<demuxer->movi_start)
    ...
    if(priv->idx_size>0)
    {
        ...
        if(sh_audio)
            sh_audio->i_bps=((float)asize/(float)asamples)*(float)sh_audio->audio.dwRate/(float)sh_audio->audio.dwScale;
    }
    else
    {
        ...
        if((priv->numberofframes=sh_video->video.dwLength)<=1)
            ...
            if(sh_audio && sh_audio->wf->nAvgBytesPerSec)
                priv->numberofframes=sh_video->fps*sh_audio->audio.dwLength/sh_audio->audio.dwRate*sh_audio->audio.dwScale;
