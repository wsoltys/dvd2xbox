...
static void setup_audio_buffer_sizes(priv_t *priv)
{
    ...
    if (priv->audio_buffer_size < 256)
        priv->audio_buffer_size = 256;

    // make the skew buffer at least 1 second long
    priv->aud_skew_cnt = 1 + 1*priv->audio_in.samplerate
                         *priv->audio_in.channels
                         *bytes_per_sample/priv->audio_in.blocksize;
    if (priv->aud_skew_cnt < 16)
        priv->aud_skew_cnt = 16;
...
static int init(priv_t *priv)
{
    ...
    if (!priv->capability.audios && !tv_param_force_audio)
        tv_param_noaudio = 1;
...
static int control(priv_t *priv, int cmd, void *arg)
{
    ...
    case TVI_CONTROL_AUD_SET_SAMPLERATE:
        {
            if (audio_in_set_samplerate(&priv->audio_in, (int)*(void **)arg) < 0)
                return TVI_CONTROL_FALSE;
            setup_audio_buffer_sizes(priv);
            return(TVI_CONTROL_TRUE);
        }
