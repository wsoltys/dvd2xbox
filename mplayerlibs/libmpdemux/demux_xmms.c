...
{
    if (xmms_priv)
        xmms_flushto=time*((INT64) xmms_byterate)/1000;
}

static int disk_free(void)
{ // vqf plugin sends more than it should
...
{
    if (xmms_byterate)
        return xmms_priv->spos*1000/((INT64)xmms_byterate);
    else
        return 0;
}
