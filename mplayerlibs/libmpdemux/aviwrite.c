...
aviwrite_stream_t* aviwrite_new_stream(aviwrite_t *muxer,int type)
{
    ...
    {
        mp_msg(0,0,"Too many streams! increase AVIWRITE_MAX_STREAMS !\n");
        return NULL;
    }
    ...
    case AVIWRITE_TYPE_AUDIO:
        ...
        break;
    default:
        mp_msg(0,0,"WarninG! unknown stream type: %d\n",type);
        return NULL;
    }
...
void aviwrite_write_chunk(aviwrite_t *muxer,aviwrite_stream_t *s, FILE *f,int len,unsigned int flags)
{
    ...
    if(s->h.dwSampleSize)
    {
        ...
        if(len%s->h.dwSampleSize)
            mp_msg(0,0,"Warning! len isn't divisable by samplesize!\n");
    }
    else
    {
        // VBR
