...
void demux_seek_mf(demuxer_t *demuxer,float rel_seek_secs,int flags)
{
    ...
    if ( flags & 2 )
        newpos+=rel_seek_secs*mf->nr_of_files;
    else
...
#ifdef WIN32
    if ( !( f=fopen( mf->names[mf->curr_frame],"rb" ) ) )
        return 0;
#else
    if ( !( f=fopen( mf->names[mf->curr_frame],"r" ) ) )
        return 0;
#endif
    {
        sh_video_t     * sh_video = demuxer->video->sh;
...
demuxer_t* demux_open_mf(demuxer_t* demuxer)
{
    ...
    if ( !strcmp( mf_type,"jpg" ) ||
            !(strcmp(mf_type, "jpeg")))
        sh_video->format = mmioFOURCC('I', 'J', 'P', 'G');
    else
        if ( !strcmp( mf_type,"png" ))
            sh_video->format = mmioFOURCC('M', 'P', 'N', 'G' );
        else
            if ( !strcmp( mf_type,"tga" ))
                sh_video->format = mmioFOURCC('M', 'T', 'G', 'A' );
            else
                if (!strcmp( mf_type,"sgi" ))
                    sh_video->format = mmioFOURCC('S', 'G', 'I', '1');
                else
                {
