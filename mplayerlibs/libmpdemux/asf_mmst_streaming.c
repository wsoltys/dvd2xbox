                          ...
                          char *data)
{
    ...
    if (socket_write (s, cmd.buf, length+48) != (length+48))
    {
        mp_msg (0,0,"write error\n");
    }
}
...
static void get_answer (int s)
{
    ...
    while (command == 0x1b)
    {
        ...
        len = socket_read (s, data, BUF_SIZE) ;
        if (!len)
        {
            mp_msg (0,0,"\nalert! eof\n");
            return;
...
static int get_data (int s, char *buf, size_t count)
{
    ...
    while (total < count)
    {
        ...
        len = socket_read (s, &buf[total], count-total);

        if (len<0)
        {
            mp_msg(0,0,"read error:");
            return 0;
        }
...
static int get_header (int s, uint8_t *header, streaming_ctrl_t *streaming_ctrl)
{
    ...
    while (1)
    {
        ...
        {
            mp_msg (0,0,"pre-header read failed\n");
            return 0;
        }
        if (pre_header[4] == 0x02)
        {
            ...
            //      mp_msg (0,0,"asf header packet detected, len=%d\n", packet_len);

            if (!get_data (s, &header[header_len], packet_len))
            {
                mp_msg (0,0,"header data read failed\n");
                return 0;
            }
            ...
            if ( (header[header_len-1] == 1) && (header[header_len-2]==1))
            {
                ...
                //	mp_msg (0,0,"get header packet finished\n");
        ...
        else
        {
            ...
            {
                mp_msg (0,0,"packet_len read failed\n");
                return 0;
            }

            packet_len = get_32 ((unsigned char*)&packet_len, 0) + 4;

            //      mp_msg (0,0,"command packet detected, len=%d\n", packet_len);

            if (!get_data (s, data, packet_len))
            {
                mp_msg (0,0,"command data read failed\n");
                return 0;
            }

            command = get_32 (data, 24) & 0xFFFF;

            //      mp_msg (0,0,"command: %02x\n", command);
        ...
        //    mp_msg (0,0,"get header packet succ\n");
    }
}
...
static int interp_header (uint8_t *header, int header_len)
{
    ...
    while (i<header_len)
    {
        ...
        //    mp_msg (0,0,"guid found: %016llx%016llx\n", guid_1, guid_2);
        ...
        else if ((guid_1 == 0x6553200cc000e48e) && (guid_2 == 0x11cfa9478cabdca1))
        {
            ...
            mp_msg (0,0,"file object, packet length = %d (%d)\n",
        ...
        else if ((guid_1 == 0x6553200cc000e68e) && (guid_2 == 0x11cfa9b7b7dc0791))
        {
            ...
            mp_msg (0,0,"stream object, stream id: %d\n", stream_id);
        ...
        {
            mp_msg (0,0,"unknown object\n");
        }

        //    mp_msg (0,0,"length    : %lld\n", length);
...
static int get_media_packet (int s, int padding, streaming_ctrl_t *stream_ctrl)
{
    ...
    if (pre_header[4] == 0x04)
    {
        ...
        //    mp_msg (0,0,"asf media packet detected, len=%d\n", packet_len);

        if (!get_data (s, data, packet_len))
        {
            mp_msg (0,0,"media data read failed\n");
            return 0;
        }
    ...
    else
    {
        ...
        {
            mp_msg (0,0,"everything done. Thank you for downloading a media file containing proprietary and patentend technology.\n");
            return -1;
        }
        ...
        {
            mp_msg (0,0,"unknown command %02x\n", command);
            return -1;
        }
    }

    //  mp_msg (0,0,"get media packet succ\n");
