...
static void get_str(int isbyte, demuxer_t *demuxer, char *buf, int buf_size)
{
    ...
    mp_msg(0,0,"read_str: %d bytes read\n", len);
}
...
static void skip_str(int isbyte, demuxer_t *demuxer)
{
    ...
    mp_msg(0,0,"skip_str: %d bytes skipped\n", len);
}
...
static void dump_index(demuxer_t *demuxer, int stream_id)
{
    ...
    mp_msg(0,0,"Index table for stream %d\n", stream_id);
    for (i = 0; i < entries; i++)
    {
        mp_msg(0,0,"packetno: %x pos: %x len: %x timestamp: %x flags: %x\n",
...
read_index:
    ...
    {
        mp_msg(0,0,"Something went wrong, no index chunk found on given address (%d)\n",
               next_header_pos);
        goto end;
    }

    mp_msg(0,0,"Reading index table from index chunk (%d)\n",
           next_header_pos);

    i = stream_read_dword(demuxer->stream);
    mp_msg(0,0,"size: %d bytes\n", i);

    i = stream_read_word(demuxer->stream);
    if (i != 0)
        mp_msg(0,0,"Hmm, index table with unknown version (%d), please report it to MPlayer developers!\n", i);

    entries = stream_read_dword(demuxer->stream);
    mp_msg(0,0,"entries: %d\n", entries);

    stream_id = stream_read_word(demuxer->stream);
    mp_msg(0,0,"stream_id: %d\n", stream_id);

    next_header_pos = stream_read_dword(demuxer->stream);
    mp_msg(0,0,"next_header_pos: %d\n", next_header_pos);
    if (entries <= 0)
    {
        if (next_header_pos)
            goto read_index;
...
read_index:
    ...
    {
        mp_msg(0,0,"Something went wrong, no data chunk found on given address (%d)\n",
               data_pos);
        goto end;
    }
    ...
    mp_msg(0,0,"Generating index table from raw data (pos: 0x%x) for %d packets\n",
...
got_audio:
            ...
            // parse audio chunk:
            {
                ...
                { // HACK - used for reverse engineering the descrambling matrix
                    ...
                    for(i=0;i<len;i++)
                        dp->buffer[i]=(i==cnt2) ? (cnt+16*(8+cnt)) : 0;
                    if(cnt==6)
                    {
                        cnt=0;
                        ++cnt2;
...
got_video:
            ...
            // parse video chunk:
            {
                ...
                while(len>2)
                {
                    ...
                    vpkg_header=stream_read_char(demuxer->stream);
                    --len;
                    ...
                    if (0x40==(vpkg_header&0xc0))
                    {
                        ...
                        int bummer;
                        bummer=stream_read_char(demuxer->stream);
                        --len;
