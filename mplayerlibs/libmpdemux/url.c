...
URL_t* url_new(char* url)
{
    ...
    if( ptr1==NULL )
    {
				if (url[0]=='$')
				{
					Curl->protocol=(char*)malloc(10);
					strcpy(Curl->protocol,"relax");
					pos1=1;
					ptr1=&url[1];
				}
				else
    ...
    if( Curl->file==NULL )
    {
        ...
        if( Curl->file==NULL )
        {
            ...
            return NULL;
				...
				if (0==strcmp(Curl->protocol,"relax") )
				{
					char* strFile = strstr(url, ":");
					if (strFile)
					{
						char* strFile2 = strstr(strFile+1, ":");
						if (strFile2)
						{
							strFile2++;
							Curl->file=realloc(Curl->file, strlen(strFile2)+1);
							strcpy(Curl->file,strFile2);
						}
						else
						{
							strFile++;
							Curl->file=realloc(Curl->file, strlen(strFile)+1);
							strcpy(Curl->file,strFile);
						}
					}
				}
				else
...
void url_debug(URL_t *url)
{
    ...
    {
        mp_msg(0,0,"protocol=%s\n", url->protocol );
    }
    if( url->hostname!=NULL )
    {
        mp_msg(0,0,"hostname=%s\n", url->hostname );
    }
    mp_msg(0,0,"port=%d\n", url->port );
    if( url->file!=NULL )
    {
    ...
    {
        mp_msg(0,0,"username=%s\n", url->username );
    }
    if( url->password!=NULL )
    {
        mp_msg(0,0,"password=%s\n", url->password );
    }
}
#endif //__URL_DEBUG
