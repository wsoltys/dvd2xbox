        ...
        if(ds->asf_packet)
        {
            ...
            else
            {
                ...
                if(dp->len!=offs && offs!=-1)
                    mp_msg(MSGT_DEMUX,MSGL_V,"warning! fragment.len=%d BUT next fragment offset=%d  \n",dp->len,offs);
...
//            mp_msg(MSGT_DEMUX,MSGL_V,"Invalid plen type! assuming plen=0\n");
        ...
        for(seg=0;seg<segs;seg++)
        {
            ...
            if(p>=p_end)
                mp_msg(MSGT_DEMUX,MSGL_V,"Warning! invalid packet 1, sig11 coming soon...\n");
...
void demux_seek_asf(demuxer_t *demuxer,float rel_seek_secs,int flags)
{
    ...
    if(newpos<0 || newpos<demuxer->movi_start)
