...
http_response_parse( HTTP_header_t *http_hdr )
{
    ...
    if( pos_hdr_sep+hdr_sep_len<(int)http_hdr->buffer_size )
    {
        // Response has data!
...
http_get_next_field( HTTP_header_t *http_hdr )
{
    ...
    while( field!=NULL )
    {
        ...
        if( ptr==NULL )
            return NULL;
        if( !strncmp( field->field_name, http_hdr->field_search, ptr-(field->field_name) ) )
        {
            ptr++;	// Skip the column
            while( ptr[0]==' ' )
...
http_add_basic_authentication( HTTP_header_t *http_hdr, const char *username, const char *password )
{
    ...
    if( http_hdr==NULL || username==NULL )
        return -1;
