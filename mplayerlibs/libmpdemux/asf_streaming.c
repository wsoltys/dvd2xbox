...
asf_streaming_start( stream_t *stream, int *demuxer_type)
{
    ...
    if( 	!strncmp( proto_s, "http", 4) ||
            (!strncmp( proto_s, "mms", 3) && strncmp( proto_s, "mmst", 4)) ||
            !strncmp( proto_s, "http_proxy", 10)
      )
    {
    ...
    if( !strncmp( proto_s, "mms", 3) && strncmp( proto_s, "mmst", 4) )
    {
...
{
    /*
    mp_msg(0,0,"ASF stream chunck size=%d\n", stream_chunck->size);
    mp_msg(0,0,"length: %d\n", length );
    mp_msg(0,0,"0x%02X\n", stream_chunck->type );
    */
    if( drop_packet!=NULL )
    ...
    /*
    	mp_msg(0,0,"  type: 0x%02X\n", stream_chunck->type );
    	mp_msg(0,0,"  size: %d (0x%02X)\n", stream_chunck->size, stream_chunck->size );
    	mp_msg(0,0,"  sequence_number: 0x%04X\n", stream_chunck->sequence_number );
    	mp_msg(0,0,"  unknown: 0x%02X\n", stream_chunck->unknown );
    	mp_msg(0,0,"  size_confirm: 0x%02X\n", stream_chunck->size_confirm );
    */
...
asf_streaming_parse_header(int fd, streaming_ctrl_t* streaming_ctrl)
{
    ...
    do
    {
        ...
        if(r)
            mp_msg(MSGT_NETWORK,MSGL_WARN,"Warning : drop header ????\n");
        if(size < 0)
        {
        ...
        if( chunk_size2read==0 )
        {
            if(size < (int)sizeof(asfh))
            {
                ...
                mp_msg(MSGT_NETWORK,MSGL_DBG2,"Got chunk\n");
    ...
    while(size - pos >= (int)sizeof(objh))
    {
        ...
        case 0x8CABDCA1 : // File header
            ...
            /*
            	...
            	mp_msg(0,0,"Error packetsize check don't match\n");
            	return -1;
                  }
            */
        ...
        case 0x7bf875ce :  // stream bitrate properties object
            mp_msg(0,0,"Stream bitrate properties object\n");
            mp_msg(0,0,"Max bandwidth set to %d\n", streaming_ctrl->bandwidth);
            asf_ctrl->audio_id = 0;
            asf_ctrl->video_id = 0;
            if( streaming_ctrl->bandwidth!=0 )
            {
                ...
                ptr += sizeof(uint16_t);
                mp_msg(0,0," stream count=[0x%x][%u]\n", stream_count, stream_count );
                for( i=0 ; i<stream_count && ptr<((char*)buffer+pos+objh.size) ; i++ )
                {
                    ...
                    {
                        total_bitrate += p_br;
                        mp_msg(0,0,"total_bitrate=%d\n", total_bitrate);
                        mp_msg(0,0,"id=%d\n", p_id);
                        break;
                    }
                    ptr += sizeof(uint32_t);
                    mp_msg(0,0,"   stream id=[0x%x][%u]\n", stream_id, stream_id );
                    mp_msg(0,0,"   max bitrate=[0x%x][%u]\n", max_bitrate, max_bitrate );
                    p_id = stream_id;
                    p_br = max_bitrate;
                }
                asf_ctrl->audio_id = 1;
...
asf_header_check( HTTP_header_t *http_hdr )
{
    ...
    if( http_hdr==NULL )
        return -1;
    if( http_hdr->body==NULL || http_hdr->body_size<sizeof(ASF_obj_header_t) )
        return -1;
...
asf_http_streaming_type(char *content_type, char *features, HTTP_header_t *http_hdr )
{
    ...
    if( 	!strcmp(content_type, "application/octet-stream") ||
            !strcmp(content_type, "application/vnd.ms.wms-hdr.asfv1") ||        // New in Corona, first request
    ...
    else
    {
        ...
        {
            if(	(!strcmp(content_type, "audio/x-ms-wax")) ||
                    (!strcmp(content_type, "audio/x-ms-wma")) ||
                    (!strcmp(content_type, "video/x-ms-asf")) ||
                    (!strcmp(content_type, "video/x-ms-afs")) ||
                    (!strcmp(content_type, "video/x-ms-wvx")) ||
                    (!strcmp(content_type, "video/x-ms-wmv")) ||
                    (!strcmp(content_type, "video/x-ms-wma")) )
            {
                mp_msg(MSGT_NETWORK,MSGL_ERR,"=====> ASF Redirector\n");
                return ASF_Redirector_e;
            }
            else if( !strcmp(content_type, "text/plain") )
            {
                mp_msg(MSGT_NETWORK,MSGL_V,"=====> ASF Plain text\n");
                return ASF_PlainText_e;
            }
...
asf_http_request(streaming_ctrl_t *streaming_ctrl)
{
    ...
    if( url==NULL || asf_http_ctrl==NULL )
        return NULL;
...
asf_http_streaming_start( stream_t *stream, int *demuxer_type )
{
    ...
    do
    {
        ...
        {
            int r = socket_write( fd, http_hdr->buffer+i, http_hdr->buffer_size-i );
            if(r <0)
            {
        ...
        do
        {
            i = socket_read( fd, buffer, BUFFER_SIZE );
            //mp_msg(0,0,"read: %d\n", i );
            if( i<=0 )
            {
                mp_msg(0,0,"read");
                ...
                if( streaming_bufferize( stream->streaming_ctrl, http_hdr->body, http_hdr->body_size )<0 )
                {
                    ...
                    return -1;
						...
						//stream->type = STREAMTYPE_PLAYLIST;
						{
							char szURL[1024];
							char *hrefend;
							char *href = strstr(http_hdr->body,"<Ref href = \"");
							href += strlen("<Ref href = \"");

							hrefend=strstr(href,"\" />");
							strncpy(szURL,href, hrefend-href);
							szURL[hrefend-href]=0;
							OutputDebugString(szURL);
							socket_close(fd);
              http_free( http_hdr );

							url_free(stream->streaming_ctrl->url);
							stream->streaming_ctrl->url=url_new(szURL);
							return -1;
						}
            done = 1;
            break;
        case ASF_Authenticate_e:
            if( http_authenticate( http_hdr, url, &auth_retry
                                 )<0 ) return -1;
            asf_http_ctrl->streaming_type = ASF_Unknown_e;
            done = 0;
            break;
