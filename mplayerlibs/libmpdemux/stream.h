...
static int stream_read_char(stream_t *s)
{
...
static unsigned int stream_read_word(stream_t *s)
{
  int x,y;
...
static unsigned int stream_read_dword(stream_t *s){
  unsigned int y;
  y=stream_read_char(s);
...
static unsigned int stream_read_word_le(stream_t *s)
{
    int x,y;
    x=stream_read_char(s);
    y=stream_read_char(s);
...
static unsigned int stream_read_dword_le(stream_t *s)
{
    unsigned int y;
    y=stream_read_char(s);
...
static uint64_t stream_read_qword(stream_t *s)
{
    uint64_t y;
    y = stream_read_char(s);
    y=(y<<8)|stream_read_char(s);
...
static unsigned int stream_read_int24(stream_t *s)
{
    unsigned int y;
    y = stream_read_char(s);
    y=(y<<8)|stream_read_char(s);
...
static int stream_read(stream_t *s,char* mem,int total)
{
  int len=total;
  while(len>0){
    int x;
...
static off_t stream_tell(stream_t *s)
{
    return s->pos+s->buf_pos-s->buf_len;
}

static int stream_seek(stream_t *s,off_t pos)
{

//    mp_dbg(MSGT_DEMUX, MSGL_DBG3, "seek to 0x%X\n",(INT64)pos);
...
static int stream_skip(stream_t *s,off_t len)
{
