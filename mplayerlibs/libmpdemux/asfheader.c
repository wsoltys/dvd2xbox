...
int asf_check_header(demuxer_t *demuxer)
{
    ...
    //  for(i=0;i<16;i++) mp_msg(0,0," %02X",temp[i]);mp_msg(0,0,"\n");
    //  for(i=0;i<16;i++) mp_msg(0,0," %02X",asfhdrguid[i]);mp_msg(0,0,"\n");
...
int read_asf_header(demuxer_t *demuxer)
{
...
#if 1
    //mp_msg(0,0,"ASF file! (subchunks: %d)\n",asfh.cno);
    while(!stream_eof(demuxer->stream))
    {
        ...
        case ASF_GUID_PREFIX_stream_header:
            ...
            // type-specific data:
            stream_read(demuxer->stream,(char*) asfbuffer,streamh.type_size);
            ...
            case ASF_GUID_PREFIX_audio_stream:
                {
                    ...
                    memcpy(sh_audio->wf,asfbuffer,streamh.type_size);
                    le2me_WAVEFORMATEX(sh_audio->wf);
                    if(verbose>=1)
                        print_wave_header(sh_audio->wf);
                    if(ASF_LOAD_GUID_PREFIX(streamh.concealment)==ASF_GUID_PREFIX_audio_conceal_interleave)
                    {
                        stream_read(demuxer->stream,(char*) asfbuffer,streamh.stream_size);
                        asf_scrambling_h=asfbuffer[0];
                        asf_scrambling_w=(asfbuffer[2]<<8)|asfbuffer[1];
                        asf_scrambling_b=(asfbuffer[4]<<8)|asfbuffer[3];
            ...
            case ASF_GUID_PREFIX_video_stream:
                {
                    ...
                    memcpy(sh_video->bih,&asfbuffer[4+4+1+2],len);
        ...
        case ASF_GUID_PREFIX_stream_group:
            {
                ...
                mp_msg(0,0,"============ ASF Stream group == START ===\n");
                mp_msg(0,0," object size = %d\n", (int)objh.size);
                object = (char*)malloc(objh.size);
                if( object==NULL )
                {
                    mp_msg(0,0,"Memory allocation failed\n");
                    return 0;
                }
                ...
                mp_msg(0,0,"============ ASF Stream group == END ===\n");
                free( object );
                break;
            }
        } // switch GUID
