    ...
    case STREAMTYPE_DVDNAV:
        {
            ...
            if (len==0)
                return 0; // this was an event, so repeat the read
            break;
        }
#endif
#ifdef USE_DVDREAD
...
int stream_seek_long(stream_t *s,off_t pos)
{
    ...
    case STREAMTYPE_STREAM:
        ...
        newpos=pos&(~((__int64)STREAM_BUFFER_SIZE-1));
        break;
#else
...
#endif
    case STREAMTYPE_VCDBINCUE:
        newpos=(pos/VCD_SECTOR_DATA)*VCD_SECTOR_DATA;
        break;
    case STREAMTYPE_DVD:
        newpos=pos/2048;
        newpos*=2048;
...
//	END BOBBIN007
    ...
    default:
        ...
        { // Otherwise on the buffer size
#ifdef _LARGEFILE_SOURCE
            newpos=pos&(~((INT64)STREAM_BUFFER_SIZE-1));
            break;
#else
newpos=pos&(~(STREAM_BUFFER_SIZE-1));
            break;
    ...
    if(verbose>=3)
    {
        ...
        printf("s->pos=%llX  newpos=%llX  new_bufpos=%llX  buflen=%X  \n",
               (INT64)s->pos,(INT64)newpos,(INT64)pos,s->buf_len);
#else
...
//	END BOBBIN007
        ...
        case STREAMTYPE_STREAM:
            ...
            if( s->streaming_ctrl!=NULL )
            {
                if( s->streaming_ctrl->streaming_seek( s->fd, s->pos, s->streaming_ctrl )<0 )
                {
...
//	END BOBBIN007
    ...
    //if(s->priv) free(s->priv);
		...
		if ( s->streaming_ctrl)
		{
			s->streaming_ctrl->streaming_close(0,s);
