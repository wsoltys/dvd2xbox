...
void read_avi_header(demuxer_t *demuxer,int index_mode)
{
    ...
    while(1)
    {
        ...
        if(id==mmioFOURCC('L','I','S','T'))
        {
            ...
            if(id==listtypeAVIMOVIE)
            {
                ...
                if(list_end>0)
                    stream_seek(demuxer->stream,list_end); // skip movi
                list_end=0;
            }
            continue;
        }
        ...
        switch(id)
        {
					...
					{
						DWORD dwTotalFrames;
						stream_read(demuxer->stream,(char*) &dwTotalFrames,sizeof(dwTotalFrames));
						chunksize-=sizeof(dwTotalFrames);	
						mp_msg(0,0,"%i==%i?\n", dwTotalFrames,avih.dwTotalFrames);
					}
					break;
            // Indicates where the subject of the file is archived
        case mmioFOURCC('I','A','R','L'): hdr="Archival Location";
            break;
            // Lists the artist of the original subject of the file;
            // for example, "Michaelangelo."
        case mmioFOURCC('I','A','R','T'): hdr="Artist";
            break;
            // Lists the name of the person or organization that commissioned
            // the subject of the file; for example "Pope Julian II."
        case mmioFOURCC('I','C','M','S'): hdr="Commissioned";
            break;
        ...
        case mmioFOURCC('I','C','M','T'): hdr="Comments";
            break;
        ...
        case mmioFOURCC('I','E','N','G'): hdr="Engineer";
            break;
            // Describes the original work, such as "landscape,", "portrait,"
            // "still liefe," etc.
        case mmioFOURCC('I','G','N','R'): hdr="Genre";
            break;
        ...
        case mmioFOURCC('I','K','E','Y'): hdr="Keywords";
            break;
        ...
        case mmioFOURCC('I','S','H','P'): hdr="Sharpness";
            break;
        ...
        case mmioFOURCC('I','S','R','C'): hdr="Source";
            break;
            ...
            // the same as IMED
        case mmioFOURCC('I','S','R','F'): hdr="Source Form";
            break;
            // ITCH - Identifies the technician who digitized the subject file;
            // for example, "Smith, John."
        case mmioFOURCC('I','T','C','H'): hdr="Technician";
            break;
        case mmioFOURCC('I','S','M','P'): hdr="Time Code";
            break;
        case mmioFOURCC('I','D','I','T'): hdr="Digitization Time";
            break;
        ...
        case ckidAVIMAINHDR:          // read 'avih'
            ...
            if(verbose>0)
                print_avih(&avih); // else print_avih_flags(&avih);
            break;
        ...
        case ckidSTREAMFORMAT:
            {      // read 'strf'
                if(last_fccType==streamtypeVIDEO)
                {
                    ...
                    if(verbose>=1)
                        print_video_header(sh_video->bih);
                    chunksize=0;
        ...
        case ckidAVINEWINDEX:
            ...
            if(index_mode)
            {
                ...
                //      printf("index to %p !!!!! (priv=%p)",priv->idx,priv);
                ...
                //if(verbose>=2)
                  //  print_index(priv->idx,priv->idx_size);
                break;
            }
            /* added May 2002 */
        ...
        if(list_end>0 && chunksize+stream_tell(demuxer->stream)>list_end)
        {
            ...
            if(chunksize>0)
                ...
                if((int)chunksize<0)
                    mp_msg(MSGT_HEADER,MSGL_WARN,"chunksize=%u  (id=%.4s)\n",chunksize,(char *) &id);
    ...
    if(index_mode>=2 || (priv->idx_size==0 && index_mode==1))
    {
        ...
        while(1)
        {
            ...
            mp_dbg(MSGT_HEADER,MSGL_DBG2,"%08X %08X %.4s %08X %X",(unsigned int)demuxer->filepos,id,(char *) &id,(int)c,(unsigned int) idx->dwFlags);
#if 0
