...
size_t y4m_read(stream_t *s, char *buf, size_t len)
{
    size_t n;
...
static int y4m_snprint_xtags(char *s, int maxn, y4m_xtag_list_t *xtags)
{
    ...
    {
        int n = _snprintf(s, room + 1, " %s", xtags->tags[i]);
        if ((n < 0) || (n > room))
...
int y4m_write_stream_header(int fd, y4m_stream_info_t *i)
{
    ...
    n = _snprintf(s, sizeof(s), "%s W%d H%d F%d:%d I%s A%d:%d",
                  Y4M_MAGIC,
                  i->width,
                  i->height,
...
int y4m_write_frame_header(int fd, y4m_frame_info_t *i)
{
    ...
    n = _snprintf(s, sizeof(s), "%s", Y4M_FRAME_MAGIC);
    if ((n < 0) || (n > Y4M_LINE_MAX))
        return Y4M_ERR_HEADER;
    if ((err = y4m_snprint_xtags(s + n, sizeof(s) - n - 1, &(i->x_tags)))
            != Y4M_OK)
                   ...
                   y4m_frame_info_t *fi, unsigned char *yuv[3])
{
    ...
    /* Read frame header */
    if ((err = y4m_read_frame_header(s, fi)) != Y4M_OK)
        return err;
    /* Read luminance scanlines */
    if (y4m_read(s, yuv[0], w*h))
        return Y4M_ERR_SYSTEM;
    /* Read chrominance scanlines */
    if (y4m_read(s, yuv[1], w*h/4))
        return Y4M_ERR_SYSTEM;
    if (y4m_read(s, yuv[2], w*h/4))
        return Y4M_ERR_SYSTEM;
...
void y4m_log_stream_info(const char *prefix, y4m_stream_info_t *i)
{
    ...
    _snprintf(s, sizeof(s), "  frame size:  ");
    if (i->width == Y4M_UNKNOWN)
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "(?)x");
    else
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "%dx", i->width);
    if (i->height == Y4M_UNKNOWN)
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "(?) pixels ");
    else
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "%d pixels ", i->height);
    if (i->framelength == Y4M_UNKNOWN)
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "(? bytes)");
    else
        _snprintf(s+strlen(s), sizeof(s)-strlen(s), "(%d bytes)", i->framelength);
