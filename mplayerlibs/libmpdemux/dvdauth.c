...
static int path_to_lba (char *path)
{
    ...
    if (fp)
    {
        ...
        else
        {
            ...
            {
                lba=atoi(cmd);
                mp_msg(0,0,"DVD: fibmap_mplayer is being used\n");
            }
            else
                fp=NULL;
        }
    ...
    {
        int fd;
        mp_msg(0,0,"DVD: fibmap_mplayer could not run, trying with ioctl() ...\n");
...
static void reset_agids ( dvd_device_t *dvd )
{
...
static int dvd_load_css_so ( void )
{
    if (css_so_is_loaded)
    {
        mp_msg(0,0,"DVD: warning: attempt to load css.so twice, ignoring.\n");
        return 0;
    }
    if (!css_so)
        css_so=strdup("libcss.so");
    mp_msg(0,0,"DVD: opening libcss.so as %s ...\n",css_so);
    dlid=dlopen(css_so,RTLD_NOW);
    if (!dlid)
    {
        mp_msg(0,0,"DVD: dlopen: %s\n",dlerror());
        return 1;
    }
    mp_msg(0,0,"DVD: dlopen OK!\n");
...
#undef CSS_DLSYM
    ...
    css_so_is_loaded=1;
    mp_msg(0,0,"DVD: shared object %s is loaded normally.\n",css_so);
    return 0;
}
#endif
...
int dvd_import_key ( unsigned char *hexkey )
{
    ...
    //	mp_msg(0,0,"DVD key: %s\n",hexkey);
    for (len=0;len<10;len++)
    {
        //		mp_msg(0,0,"-> %c\n",*hexkey);
        if (!*hexkey)
            return 1;
        if (*hexkey>='A'&&*hexkey<='F')
            *t|=(*hexkey-'A'+10)<<digit;
        else if (*hexkey>='0'&&*hexkey<='9')
            *t|=(*hexkey-'0')<<digit;
        else
            return 1;
        if (digit)
...
{
    dvd_device_t *dvd;  /* DVD device handle */
	int i;

//    if (dvd_load_css_so())
//        return 1;

//	OutputDebugString("Authenticating DVD\n");
    if ((dvd=DVDOpenDevice(dev)) == NULL)
    {
//		OutputDebugString("Failed to open");
//		OutputDebugString(dev);
//		OutputDebugString("\n");
    ...
    if (!CSSisEncrypted(dvd))
    {
//		OutputDebugString("Drive is not encrypted!\n");
//        mp_msg(0,0,"DVD: DVD is unencrypted! Skipping authentication!\n(note: you should not use -dvd switch for unencrypted discs!)\n");
        DVDCloseDevice(dvd);
        return 0;
    }
//    else
//        mp_msg(0,0,"DVD: DVD is encrypted, issuing authentication ...\n");
    /* reset AGIDs */
    reset_agids(dvd);
    /* authenticate disc */
//	OutputDebugString("Going to authenticate disc\n");
    if (CSSAuthDisc(dvd,key_disc))
    {
//		OutputDebugString("Authentication failed!\n");
        fprintf(stderr,"DVD: CSSDVDAuthDisc() failed.\n");
        DVDCloseDevice(dvd);
        return 1;
    }
//	OutputDebugString("CSSAuthTitle\n");
    if (CSSAuthTitlePath(dvd,key_title,filename))
    {
//		OutputDebugString("CSSAuthTitlePath failed\n");
    ...
    /* decrypting title */
//	OutputDebugString("CSSDecryptTitleKey\n");
    if (CSSDecryptTitleKey(key_title, key_disc) < 0)
    {
//		OutputDebugString("CSSDecryptTitleKey failed!\n");
        fprintf(stderr,"DVD: CSSDecryptTitleKey() failed.\n");
        DVDCloseDevice(dvd);
        return 1;
    }
	OutputDebugString("Title Key: ");
	for( i = 0; i < 6; i++ )
	{
		char buffer[20];
		sprintf(buffer, "%02X ", key_title[i]);
		OutputDebugString(buffer);
	}
	OutputDebugString("\n");

    DVDCloseDevice(dvd);
//    mp_msg(0,0,"DVD: DVD title key is: %02X%02X%02X%02X%02X\n",key_title[0],key_title[1],key_title[2],key_title[3],key_title[4]);
//	OutputDebugString("Dvd decryption enabled\n");
    descrambling=1;
    return 0;
}
