...
//#endif
                ...
                if(hdrlen>len)
                {
                    mp_msg(MSGT_DEMUX,MSGL_V,"demux_mpg: invalid header length  \n");
                    return -1;
                }
                ...
                if(id==0x1BD)
                {
                    int aid=stream_read_char(demux->stream);
                    --len;
                    if(len<3)
                    ...
                    else if((aid & 0xC0) == 0x80 || (aid & 0xE0) == 0x00)
                    {
                        ...
                        if(demux->audio->id==aid)
                        {
                            ...
                            if((aid&0xE0)==0xA0 && len>=3)
                            {
                                ...
                                len-=3;
                                if(len<=0)
                                    mp_msg(MSGT_DEMUX,MSGL_V,"End of packet while searching for PCM header\n");
                            }
                            //        printf("  \n");
                        } //  if(demux->audio->id==aid)
                        ...
                        mp_msg(MSGT_DEMUX,MSGL_V,"Unknown 0x1BD substream: 0x%02X  \n",aid);
    ...
    if(mpeg_pts_error)
        mp_msg(MSGT_DEMUX,MSGL_V,"  {PTS_err:%d}  \n",mpeg_pts_error);
    ...
    if(id>=0x1C0 && id<=0x1DF)
    {
        ...
        if(!demux->a_streams[aid])
            new_sh_audio(demux,aid);
        if(demux->audio->id==-1 || aid < demux->audio->id)
            demux->audio->id=aid;
        if(demux->audio->id==aid)
        {
            ds=demux->audio;
    ...
    if(ds)
    {
        ...
        //    mp_msg(0,0,"packet start = 0x%X  \n",stream_tell(demux->stream)-packet_start_pos);
//#ifdef HAVE_LIBCSS
        if (css)
        {
            if (descrambling)
                dvd_css_descramble(demux->stream->buffer,key_title);
            else
                    ...
                    if((head&~0x60) == 0x102)
                        ...
                        if((head&~0x60) == 0x103)
                            ...
                            if((head&~0x60) == 0x104)
                                ...
                                if((head&~0x60) == 0x105 && head != 0x105)
                                    ++num_h264_idr;
                                else
                                    if((head&~0x60) == 0x107 && head != 0x107)
                                        ++num_h264_sps;
                                    else
                                        if((head&~0x60) == 0x108 && head != 0x108)
                                            ++num_h264_pps;
...
void demux_seek_mpg(demuxer_t *demuxer,float rel_seek_secs,int flags)
{
    ...
    {
        if(demuxer->stream->type!=STREAMTYPE_VCD)
            demuxer->movi_start=0; // for VCD
        if(newpos<demuxer->movi_start)
            newpos=demuxer->movi_start;
    }

#ifdef _LARGEFILE_SOURCE
    newpos&=~((__int64)STREAM_BUFFER_SIZE-1);  /* sector boundary */
#else
...
int demux_mpg_control(demuxer_t *demuxer,int cmd, void *arg)
{
    ...
    /*    sh_audio_t *sh_audio=d_audio->sh;*/
    sh_video_t *sh_video=d_video->sh;
		if (!sh_video) return DEMUXER_CTRL_NOTIMPL;
    switch(cmd)
    {
