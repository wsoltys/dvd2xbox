...
void cache_stats(cache_vars_t* s)
{
    ...
    mp_msg(0,0,"0x%06X  [0x%06X]  0x%06X   ",(int)s->min_filepos,(int)s->read_filepos,(int)s->max_filepos);
...
int cache_read(cache_vars_t* s,unsigned char* buf,int size)
{
    ...
    while(size>0)
    {
        ...
        if(s->read_filepos>=s->max_filepos || s->read_filepos<s->min_filepos)
        {
            ...
            Sleep(10);//usec_sleep(READ_USLEEP_TIME); // 10ms
            continue; // try again...
        }
...
#else
    m_bThreadExit=TRUE;
    while (m_bThreadRunning) Sleep(50);//TerminateThread((HANDLE)s->cache_pid,0);
    free(c->stream);
#endif
    if(!c)
        return;
...
#else
{
    ...
    stream->cache_pid = CreateThread(NULL,0,ThreadProc,s,0,&threadId);
        ...
        {
            mp_msg(MSGT_CACHE,MSGL_STATUS,"\rCache prefill: %5.2f%% (%d bytes)    ",
...
static DWORD WINAPI ThreadProc(void*s)
{
    ...
    mp_msg(0,0,"cache:thread stopped");
    return 0;
}
