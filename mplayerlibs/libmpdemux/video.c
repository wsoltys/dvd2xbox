...
int video_read_properties(sh_video_t *sh_video)
{
    ...
    case DEMUXER_TYPE_RTP:
        ...
        // for MPEG headers:
        if (!demux_is_mpeg_rtp_stream(d_video->demuxer))
            break;
    ...
    case DEMUXER_TYPE_MPEG_PS:
        {
            ...
            if(sync_video_packet(d_video)==0x1B5)
            { // next packet is seq. ext.
                //    videobuf_len=0;
...
static void process_userdata(unsigned char* buf,int len)
{
    ...
    if(verbose<2)
        return;
    mp_msg(0,0, "user_data: len=%3d  %02X %02X %02X %02X '",
    ...
    *start=NULL;
      ...
      )
    {
        ...
        *start=videobuffer;
        ...
        if((int)(sh_video->fps*10000+0.5)!=picture.fps)
