...
demuxer_t * demux_open_pva (demuxer_t * demuxer)
{
    ...
    if(demuxer->stream->type!=STREAMTYPE_FILE && demuxer->stream->type!=STREAMTYPE_STREAM)
    {
        mp_msg(0,0,"demux_pva.c:not seekable");
        demuxer->seekable=0;
    }
    else
        demuxer->seekable=1;
...
int demux_pva_fill_buffer (demuxer_t * demux)
{
    ...
    while(!done)
    {
        if(!pva_get_payload(demux,&current_payload))
            return 0;
...
int demux_pva_control(demuxer_t *demuxer,int cmd, void *arg)
{
