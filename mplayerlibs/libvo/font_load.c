...
raw_file* load_raw(char *name,int verbose)
{
    ...
    if(fread(head,32,1,f)<1)
        return NULL;        // too small
    if(memcmp(head,"mhwanh",6))
        return NULL;        // not raw file
    ...
    if(raw->c>256)
        return NULL;                 // too many colors!?
    //if(verbose) mp_msg(0,0,"RAW: %s  %d x %d, %d colors\n",name,raw->w,raw->h,raw->c);
    if(raw->c)
    {
...
font_desc_t* read_font_desc(char* strDir, char* fname,float factor,int verbose)
{
    ...
    if(!f)
    {
        mp_msg(0,0,"font: can't open file: %s\n",fname);
        return NULL;
    }
    ...
    desc->fpath = strDir; // search in the same dir as fonts.desc
    ...
    while(fgets(sor,1020,f))
    {
        ...
        *d=0;
        ...
        //  mp_msg(0,0,"params=%d  sor=%s\n",pdb,sor);
        //  for(i=0;i<pdb;i++) mp_msg(0,0,"  param %d = '%s'\n",i,p[i]);
        ...
        if(pdb==1 && p[0][0]=='[')
        {
            ...
            if(len && len<63 && p[0][len-1]==']')
            {
                strcpy(section,p[0]);
                if(verbose)
                    mp_msg(0,0,"font: Reading section: %s\n",section);
                if(strcmp(section,"[files]")==0)
                {
                    ++fontdb;
                    if(fontdb>=16)
                    {
                        mp_msg(0,0,"font: Too many bitmaps defined!\n");
                        return NULL;
                    }
                }
                continue;
        ...
        if(strcmp(section,"[fpath]")==0)
        {
            ...
            if(strcmp(section,"[files]")==0)
            {
                char *default_dir=strDir;
                if(pdb==2 && strcmp(p[0],"alpha")==0)
                {
                    char *cp;
                    if (!(cp=malloc(strlen(strDir)+strlen(p[1])+2)))
                        return NULL;

                    _snprintf(cp,strlen(strDir)+strlen(p[1])+2,"%s\\%s", strDir,p[1]);
                    if(!((desc->pic_a[fontdb]=load_raw(cp,verbose))))
                    {
                        ...
                        if (!((desc->pic_a[fontdb]=load_raw(cp,verbose))))
                        {
                            mp_msg(0,0,"Can't load font bitmap: %s\n",cp);
                            free(cp);
                            return NULL;
                        }
                    }
                ...
                if(strcmp(section,"[info]")==0)
                {
                    ...
                    if(strcmp(section,"[characters]")==0)
                    {
                        if(pdb==3)
                        {
                            ...
                            int start=atoi(p[1]);
                            int end=atoi(p[2]);
                            if(sub_unicode && (chr>=0x80))
                                chr=(chr<<8)+p[0][1];
                            else if(strlen(p[0])!=1)
                                chr=strtol(p[0],NULL,0);
                            if(end<start)
                            {
                                mp_msg(0,0,"error in font desc: end<start for char '%c'\n",chr);
                            }
                            else
                            {
                                ...
                                //              mp_msg(0,0,"char %d '%c'  start=%d width=%d\n",chr,chr,desc->start[chr],desc->width[chr]);
                                ++chardb;
                            }
                            continue;
                        }
                    }
        mp_msg(0,0,"Syntax error in font desc: %s\n",sor);

    }
    fclose(f);

    //mp_msg(0,0,"font: pos of U = %d\n",desc->start[218]);

    for(i=0;i<=fontdb;i++)
    {
        if(!desc->pic_a[i] || !desc->pic_b[i])
        {
            mp_msg(0,0,"font: Missing bitmap(s) for sub-font #%d\n",i);
            return NULL;
        }
        //if(factor!=1.0f)
        {
            ...
            int j;
            if(verbose)
                mp_msg(0,0,"font: resampling alpha by factor %5.3f (%d) ",factor,f);
            fflush(stdout);
            for(j=0;j<size;j++)
            {
...
x=255-((x*f)>>8); // scale
    ...
    mp_msg(0,0,"Font %s loaded successfully! (%d chars)\n",fname,chardb);
