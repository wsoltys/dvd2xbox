...
static int get_height(int c,int h)
{
    int font;
    if ((font=vo_font->font[c])>=0)
        if(h<vo_font->pic_a[font]->h)
...
// renders the buffer
static void vo_draw_text_from_buffer(mp_osd_obj_t* obj,void (*draw_alpha)(int x0,int y0, int w,int h, unsigned char* src, unsigned char *srca, int stride))
{
    if (obj->allocated > 0)
    {
...
static void vo_update_text_osd(mp_osd_obj_t* obj,int dxs,int dys)
{
...
static void vo_update_text_progbar(mp_osd_obj_t* obj,int dxs,int dys)
{
...

    {
        ...
        if (obj->bbox.x2 - obj->bbox.x1 < minw)
...
static void vo_update_text_sub(mp_osd_obj_t* obj,int dxs,int dys)
{
    unsigned char *t;
    ...
    while (l)
    {
        ...
        for (j=0;j<=len;j++)
        {
            ...
            { // not the last line or not the last char
                lastStripPosition=-1;
                xsize=-vo_font->charspace;
                lasth=h;
                h=vo_font->height;
...
static void vo_update_spudec_sub(mp_osd_obj_t* obj, int dxs, int dys)
{
    unsigned int bbox[4];
...
static void vo_draw_spudec_sub(mp_osd_obj_t* obj, void (*draw_alpha)(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca, int stride))
{
