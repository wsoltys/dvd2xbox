...
int geometry
(int *xpos, int *ypos, int *widw, int *widh, int scrw, int scrh)
{
    ...
    if(vo_geometry
            != NULL)
    {
        if(sscanf(vo_geometry
                  , "%ix%i+%i+%i", &width, &height, &xoff, &yoff) != 4 )
        {
            RESET_GEOMETRY
            if(sscanf(vo_geometry
                      , "%ix%i", &width, &height) != 2)
        ...
        if(xper >= 0 && xper <= 100)
            xoff = (scrw - *widw) * ((float)xper / 100.0);
        if(yper >= 0 && yper <= 100)
            yoff = (scrh - *widh) * ((float)yper / 100.0);

        /* FIXME: better checking of bounds... */
        if(width < 0 || width > scrw)
            width = *widw;
        if(height < 0 || height > scrh)
            height = *widh;
        if(xoff < 0 || xoff + *widw > scrw)
            xoff = 0;
        if(yoff < 0 || yoff + *widh > scrh)
            yoff = 0;
