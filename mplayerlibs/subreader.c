...
int xboxstrncasecmp (const char* str1,const char* str2, int iLen)
{
	int i;
	if ( strlen(str1) <iLen || strlen(str2) < iLen ) return 1;
	for (i=0; i < strlen(str1) && i < iLen;i++ )
	{
...
subtitle *sub_read_line_sami(FILE *fd, subtitle *current)
{
    ...
    do
    {
        ...
        case 3: /* get all text until '<' appears */
            if (*s == '\0')
                break;
            else if (!xboxstrncasecmp (s, "<br>", 4))
            {
                *p = '\0';
                p = text;
                trail_space (text);
...
void dump_srt(sub_data* subd, float fps)
{
    ...
    for(i=0; i < subd->sub_num; i++)
    {
        ...
        temp -= sub_delay * 100;
        h=temp/360000;
        temp%=360000;	//h =1*100*60*60
        m=temp/6000;
        temp%=6000;	//m =1*100*60
        s=temp/100;
        temp%=100;	//s =1*100
        ms=temp*10;			//ms=1*10
