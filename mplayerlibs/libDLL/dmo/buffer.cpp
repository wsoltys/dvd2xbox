...
static HRESULT __stdcall CMediaBuffer_SetLength(IMediaBuffer* This,
        unsigned long cbLength)
{
    CMediaBuffer* cmb = (CMediaBuffer*) This;
    //mp_msg(0,0,"CMediaBuffer_SetLength(%p) called (%ld, %ld)\n", This, cbLength, cmb->maxlen);
    if (cbLength > cmb->maxlen)
		{
				mp_msg(0,0,"failed CMediaBuffer_SetLength\n");
        return E_INVALIDARG;
		}
    cmb->len = cbLength;
    return S_OK;
...
static HRESULT __stdcall CMediaBuffer_GetMaxLength(IMediaBuffer* This,
        /* [out] */ unsigned long *pcbMaxLength)
{
    CMediaBuffer* cmb = (CMediaBuffer*) This;
    //mp_msg(0,0,"CMediaBuffer_GetMaxLength(%p) called -> %ld\n", This, cmb->maxlen);
    if (!pcbMaxLength)
		{
				mp_msg(0,0,"failed CMediaBuffer_GetMaxLength\n");
        return E_POINTER;
		}
...
static HRESULT __stdcall CMediaBuffer_GetBufferAndLength(IMediaBuffer* This,
        /* [out] */ char** ppBuffer,
        /* [out] */ unsigned long* pcbLength)
{
    CMediaBuffer* cmb = (CMediaBuffer*) This;
    //mp_msg(0,0,"CMediaBuffer_GetBufferAndLength(%p) called -> %p %ld\n", This, cmb->mem, cmb->len);
    if (!ppBuffer && !pcbLength)
		{
				mp_msg(0,0,"failed CMediaBuffer_GetBufferAndLength\n");
        return E_POINTER;
		}
    if (ppBuffer)
        *ppBuffer = (char*)cmb->mem;
    if (pcbLength)
        *pcbLength = cmb->len;
    return S_OK;
}
...
{
    //mp_msg(0,0,"CMediaBuffer_Destroy(%p) called\n", This);
    if (This->freemem)
        free(This->mem);
    free(This->vt);
    free(This);
