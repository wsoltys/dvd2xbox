...
(void)
{
    if(regs)
    {
        mp_msg(0,0,"Logic error: create_registry() called with existing registry\n");
        save_registry
        ();
        return;
    }
...
(void)
{
    ...
    if(regs)
    {
        mp_msg(0,0,"Multiple open_registry(>\n");
        return;
    }
    fd = open(localregpathname, O_RDONLY|O_BINARY);
    if (fd == -1)
    {
        //mp_msg(0,0,"Creating new registry\n");
        create_registry
        ();
        return;
    }
...
(void)
{
    ...
    if (fd == -1)
    {
        mp_msg(0,0,"Failed to open registry file '%s' for writing.\n",
               localregpathname);
        return;
    }
    write(fd, &reg_size, 4);
...
long RegOpenKeyExA(long key, const char* subkey, long reserved, long access, int* newkey)
{
    ...
    */
    ...
    if(!full_name)
        return -1;
    //mp_msg(0,0,"Opening key Fullname %s\n", full_name);
