...
typedef  long  ( __stdcall *GETCLASS) (const GUID*, const GUID*, void**);

//void trapbug();
extern "C" void mp_msg( int x, int lev,const char *format, ... );

extern "C" FARPROC __stdcall GetProcAddress(HINSTANCE hModule,LPCSTR lpProcName);
extern "C" HINSTANCE __stdcall LoadLibraryA(LPCSTR lpLibFileName);
extern "C" BOOL __stdcall FreeLibrary(HINSTANCE hLibModule);
extern void CodecAlloc(void);
extern void CodecRelease(void);
...
static void DS_Filter_Start(DS_Filter* This)
{
    ...
    //mp_msg(0,0,"DS_Filter_Start(%p)\n", This);
    hr = This->m_pFilter->vt->Run(This->m_pFilter, (REFERENCE_TIME)0);
    if (hr != 0)
    {
        mp_msg(0,0,"WARNING: m_Filter->Run() failed, error code %x\n", (int)hr);
    }
    hr = This->m_pImp->vt->GetAllocator(This->m_pImp, &This->m_pAll);

    if (hr || !This->m_pAll)
    {
        mp_msg(0,0,"WARNING: error getting IMemAllocator interface %x\n", (int)hr);
...
void DS_Filter_Destroy(DS_Filter* This)
{
    ...
    if (This->m_iHandle)
        FreeLibrary((HINSTANCE)This->m_iHandle);
                           ...
                           AM_MEDIA_TYPE* out_fmt)
{
    ...
    for (;;)
    {
        ...
        func = (GETCLASS)GetProcAddress((HINSTANCE)This->m_iHandle, "DllGetClassObject");
        if (!func)
        {
        ...
        //mp_msg(0,0,"Pins enumeration returned %ld pins, error is %x\n", fetched, (int)result);
        ...
        result = This->m_pInputPin->vt->QueryInterface((IUnknown*)This->m_pInputPin,&IID_IMemInputPin,(void**)&This->m_pImp);
        if (result)
        {
    ...
    if (!init)
    {
        ...
        mp_msg(0,0,"Warning: DS_Filter() %s.  (DLL=%.200s, r=0x%x)\n", em, dllname, result);
        This = 0;
    }
    return This;
}
