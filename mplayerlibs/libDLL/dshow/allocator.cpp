...
static  int avm_list_print(avm_list_t* head)
{
    ...
    if (it)
    {
        ...
        {
            i++;
            //mp_msg(0,0,"%d:  member: %p    next: %p  prev: %p\n",i, it->member, it->next, it->prev);
            it = it->next;
            if (it == head)
                break;
        }
...
static HRESULT __stdcall MemAllocator_SetProperties(IMemAllocator * This,
        /* [in] */ ALLOCATOR_PROPERTIES *pRequest,
        /* [out] */ ALLOCATOR_PROPERTIES *pActual)
{
    MemAllocator* me = (MemAllocator*)This;
    //mp_msg(0,0,"MemAllocator_SetProperties(%p) called %i %i %i %i %i\n", This, pRequest->cbAlign,pRequest->cbBuffer, pRequest->cbPrefix,pRequest->cBuffers);
    if (!pRequest || !pActual)
		{
				mp_msg(0,0,"failed MemAllocator_SetProperties\n");
        return E_INVALIDARG;
		}
...
static HRESULT __stdcall MemAllocator_GetProperties(IMemAllocator * This,
        /* [out] */ ALLOCATOR_PROPERTIES *pProps)
{
    //mp_msg(0,0,"MemAllocator_GetProperties(%p) called\n", This);
    if (!pProps)
		{
				mp_msg(0,0,"failed MemAllocator_GetProperties\n");
...
static HRESULT __stdcall MemAllocator_Commit(IMemAllocator * This)
{
    MemAllocator* me = (MemAllocator*)This;
    int i;
    //mp_msg(0,0,"MemAllocator_Commit(%p) called\n", This);
    if (((MemAllocator*)This)->props.cbBuffer < 0)
		{
				mp_msg(0,0,"failed MemAllocator_Commit\n");
        return E_FAIL;
		}
    ...
    for (i = 0; i < me->props.cBuffers; i++)
    {
        ...
        //mp_msg(0,0,"FREEEEEEEEEEEE ADDED %p\n", sample);
        me->free_list = avm_list_add_tail(me->free_list, sample);
        //avm_list_print(me->free_list);
    }

    //mp_msg(0,0,"Added mem %p: lsz: %d  %d  size: %d\n", me, avm_list_size(me->free_list), me->props.cBuffers, me->props.cbBuffer);

    return 0;
}

static HRESULT __stdcall MemAllocator_Decommit(IMemAllocator * This)
{
    MemAllocator* me=(MemAllocator*)This;
    //mp_msg(0,0,"MemAllocator_Decommit(%p) called\n", This);
 //   //mp_msg(0,0,"Deleted mem %p: %d  %d\n", me, me->free_list.size(), me->used_list.size());
    while (me->used_list)
    {
...
static HRESULT __stdcall MemAllocator_GetBuffer(IMemAllocator * This,
        ...
        /* [in] */ DWORD dwFlags)
{
    MemAllocator* me = (MemAllocator*)This;
    CMediaSample* sample;
    //mp_msg(0,0,"MemAllocator_ReleaseBuffer(%p) called   %d  %d\n", This,avm_list_size(me->used_list), avm_list_size(me->free_list));

    if (!me->free_list)
    {
        mp_msg(0,0,"failed MemAllocator_GetBuffer No samples available\n");
...
static HRESULT __stdcall MemAllocator_ReleaseBuffer(IMemAllocator* This,
        /* [in] */ IMediaSample* pBuffer)
{
    ...
    if (l)
    {
        ...
        //mp_msg(0,0,"****************** RELEASED OK %p  %p\n", me->used_list, me->free_list);
        return 0;
    }
    //mp_msg(0,0,"MemAllocator_ReleaseBuffer(%p) releasing unknown buffer!!!! %p\n", This, pBuffer);
    return E_FAIL;
}
...
{
    //mp_msg(0,0,"MemAllocator_Destroy(%p) called  (%d, %d)\n", This, This->refcount, AllocatorKeeper);
...
MemAllocator* MemAllocatorCreate()
{
    ...
    //mp_msg(0,0,"MemAllocatorCreate() called -> %p\n", This);
