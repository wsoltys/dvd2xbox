...
int main(int argc,char* argv[])
{
    ...
    printf("frame len = %d\n",len);
		...
		dst=(char*)malloc(bih.biWidth*bih.biHeight*3);

    codec=DS_VideoDecoder_Open("divx_c32.ax", &CLSID_DivxDecompressorCF, &bih, 0, 0);

    //    DS_VideoDecoder_SetDestFmt(16,fccYUY2);
    DS_VideoDecoder_SetDestFmt(codec,24,0);

    printf("DivX setting result = %d\n", DS_SetAttr_DivX("Quality",4) );

    DS_VideoDecoder_StartInternal(codec);


    DS_VideoDecoder_DecodeInternal(codec,src, len, 1, dst);
