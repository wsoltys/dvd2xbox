 ...
 *
 * $Id: highlight.c,v 1.1 2003/11/04 18:28:59 wiso Exp $
 *
 */
...
btni_t *__get_current_button(dvdnav_t *this) {
  ...
  if(dvdnav_get_current_highlight(this, &button) != S_OK) {
    mp_msg(0,0,"Unable to get information on current highlight.");
    return NULL;
  }
#ifdef BUTTON_TESTING
...
dvdnav_status_t dvdnav_get_highlight_area(pci_t* nav_pci , int32_t button, int32_t mode, 
  ...
  btni_t *button_ptr;
#ifdef BUTTON_TESTING
  mp_msg(0,0,"Button get_highlight_area %i\n", button);
#endif

  /* Set the highlight SPRM if the passed button was valid*/
  if((button <= 0) || (button > nav_pci->hli.hl_gi.btn_ns)) {
    mp_msg(0,0,"Unable to select button number %i as it doesn't exist\n",
              button);
    return S_ERR;
  }
  button_ptr = &nav_pci->hli.btnit[button-1];

  // Reassemble buttong locations because of the fact that msvc++ sucks and packing
  // Undo this change as we figured out ms weird ass packing
...
#ifdef BUTTON_TESTING
  mp_msg(0,0,"highlight.c:Highlight area is (%u,%u)-(%u,%u), display = %i, button = %u\n",
 ...
 */
   ...
   * scedule a jump */
#ifdef BUTTON_TESTING
  mp_msg(0,0, "libdvdnav: Evaluating Button Activation commands.\n");
#endif
...
dvdnav_status_t dvdnav_button_select(dvdnav_t *this, int button) {
  ...
  if(!this) {
   mp_msg(0,0,"Unable to select button number %i as this state bad",
	      button);
   return S_ERR;
  }
 
#ifdef BUTTON_TESTING
  mp_msg(0,0,"libdvdnav: Button select %i\n", button); 
#endif
