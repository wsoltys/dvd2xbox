...
static int findDirFile( const char *path, const char *file, char *filename ) 
{
	...
	dir = FindFirstFile(path, &findInfo);
	...
	do {
		...
		if( !stricmp( findInfo.cFileName, file ) )
	...
	} while( FindNextFile(dir, &findInfo) );
...
static dvd_file_t *DVDOpenSingleVobUDF( dvd_reader_t *dvd, char* file )
{
    char filename[ MAX_UDF_FILE_NAME_LEN ];
    uint32_t start, len;
    dvd_file_t *dvd_file;
	...
	int set;
	int menu;
	if(sscanf(file,"d:\\VIDEO_TS\\VTS_%02i_%i.VOB",&title,&set) > 1)
	{
		if(set)
			menu = 0;
		else
    ...
    if( !menu ) {
        ...
        //for( cur = 2; cur < 10; cur++ ) {
            sprintf( filename, "/VIDEO_TS/VTS_%02d_%d.VOB", title, set );
...
static dvd_file_t *DVDOpenSingleVobPath( dvd_reader_t *dvd, char* file )
{
    char filename[ MAX_UDF_FILE_NAME_LEN ];
    char full_path[ PATH_MAX + 1 ];
    struct stat fileinfo;
    ...
    if( menu ) {
        ...
        sprintf( filename, "VTS_%02i_%i.VOB", title,  set );
        if( findDVDFile( dvd, filename, full_path ) ) 
		{
			if( stat( full_path, &fileinfo ) < 0 ) {
				fprintf( stderr, "libdvdread: Can't stat() %s.\n", filename );
				...
				dvd_file->title_sizes[ 0 ] = fileinfo.st_size / DVD_VIDEO_LB_LEN;
				dvd_file->title_devs[ 0 ] = dvdinput_open( full_path );
				dvdinput_title( dvd_file->title_devs[ 0 ], 0 );
				dvd_file->filesize = dvd_file->title_sizes[ 0 ];
			}
		}
...
dvd_file_t *DVDOpenSingleFile( dvd_reader_t *dvd, char* file )
{
	int title;
	int set;
	char temp[30];
	...
	{
		if( dvd->isImageFile ) 
			return DVDOpenFileUDF(dvd,"/VIDEO_TS/VIDEO_TS.VOB");
		else
			return DVDOpenFilePath(dvd,"/VIDEO_TS/VIDEO_TS.VOB");
	}
	else
	{
		sscanf(file,"d:\\VIDEO_TS\\VTS_%02i_%i.VOB",&title,&set);
		sprintf(temp,"/VIDEO_TS/VTS_%02i_%i.VOB",title,set);
		if( dvd->isImageFile ) 
			return DVDOpenFileUDF(dvd,temp);
		else
			return DVDOpenFilePath(dvd,temp);
	}
