...
/* why do it this way?  We need to read lots of sectors to kludge
  ...
  long driftcomp=(long)( (float)p->dyndrift/CD_FRAMEWORDS+.5 );
  c_block *new=NULL;
  root_block *root=&p->root;
  int16_t *buffer=NULL;
  char *flags=NULL;
  ...
  while(sofar<totaltoread){
    ...
    if(secread>0){
	  ...
	  /* AF: added escape, incase abort has been pressed */
	  if ( -999 == thisread )
	  {
  ...
  while(rv(root)==NULL ||
	...
	if(p->dynoverlap==MAX_SECTOR_OVERLAP*CD_FRAMEWORDS ||
	  ...
	  if(p->stage1.offpoints!=-1){ /* hack */
	    p->dynoverlap=(long)( p->dynoverlap * 1.5 );
	    if(p->dynoverlap>MAX_SECTOR_OVERLAP*CD_FRAMEWORDS)
