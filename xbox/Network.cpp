...
bool CNetwork::Initialize(int iAssignment, const char* szLocalAddress, const char* szLocalSubnet, const char* szLocalGateway, const char* szNameServer)
{
  ...
  { /* okey now startup the settings we wish to use */
    ...
    if( dwState != 0 )
    {
      DebugOut(" - XNetStartup failed with error %d", dwState);
      return false;
    }
  }
  ...
  if( NO_ERROR != dwState )
  {
    DebugOut(" - WSAStartup failed with error %d", dwState);
    return false;
  }
...
DWORD CNetwork::UpdateState()
{
  ...
  if( m_laststate != dwState )
  {
    ...
    if ( dwState & XNET_GET_XNADDR_ETHERNET )
      DebugOut(" - State: ethernet");

    if ( dwState & XNET_GET_XNADDR_NONE )
      DebugOut(" - State: none");

    if ( dwState & XNET_GET_XNADDR_ONLINE )
      DebugOut(" - State: online");

    if ( dwState & XNET_GET_XNADDR_PENDING )
      DebugOut(" - State: pending");

    if ( dwState & XNET_GET_XNADDR_TROUBLESHOOT )
      DebugOut(" - State: error");

    if ( dwState & XNET_GET_XNADDR_PPPOE )
      DebugOut(" - State: ppoe");


    DebugOut(" - Interface: IP         %s", m_networkinfo.ip);
    DebugOut(" - Interface: SUBNET     %s", m_networkinfo.subnet);
    DebugOut(" - Interface: GATEWAY    %s", m_networkinfo.gateway);
  //  DebugOut(" - DHCPSERVER: %s", m_networkinfo.dhcpserver);
    DebugOut(" - Interface: DNS1       %s", m_networkinfo.DNS1);
    DebugOut(" - Interface: DNS2       %s", m_networkinfo.DNS2);
