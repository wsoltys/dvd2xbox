...
//-----------------------------------------------------------------------------
// Name: DrawText()
// Desc: Draws text as textured polygons vertical
//-----------------------------------------------------------------------------
HRESULT CXBFont::DrawTextVertical( FLOAT fOriginX, FLOAT fOriginY, DWORD dwColor,
                           const WCHAR* strText, DWORD dwFlags,
                           FLOAT fMaxPixelWidth )
{
    // Set up stuff (i.e. lock the vertex buffer) to prepare for drawing text
    ...
    if( dwFlags & XBFONT_CENTER_Y )
    {
        FLOAT w, h;
        GetTextExtent( strText, &w, &h );
        m_fCursorX = floorf( m_fCursorX - h/2 );
    }
    ...
    while( *strText )
    {
        ...
        if( bStartingNewLine )
        {
            if( dwFlags & (XBFONT_RIGHT|XBFONT_CENTER_X) )
            {
                ...
                // Offset this line's starting m_fCursorX value
                if( dwFlags & XBFONT_RIGHT )
                    m_fCursorY = floorf( fOriginY - w );
                if( dwFlags & XBFONT_CENTER_X )
                    m_fCursorY = floorf( fOriginY - w/2 );
            }
            bStartingNewLine = FALSE;
        }
        ...
        {
            // Check if we will be exceeded the max allowed width
            if( m_fCursorY + fOffset + fWidth + fEllipsesPixelWidth + m_fSlantFactor > fOriginY + fMaxPixelWidth )
            {
                // Yup. Let's draw the ellipses, then bail
                DrawTextVertical( m_fCursorX, m_fCursorY, dwColor, L"..." );
                End();
                return S_OK;
            }
        }
		...
		m_fCursorY		+= fOffset;
		FLOAT left		= m_fCursorX;
		FLOAT right		= left + fHeight;
		FLOAT top1		= m_fCursorY;
		FLOAT top2		= top1 + m_fSlantFactor;
		FLOAT bottom1	= top1 + fWidth;
		FLOAT bottom2	= top2 + fWidth;
        m_fCursorY		+= fAdvance;

		D3DDevice::SetVertexData4f( D3DVSDE_VERTEX, left,  bottom1, pGlyph->tu2, pGlyph->tv2 );
        D3DDevice::SetVertexData4f( D3DVSDE_VERTEX, left,  top1,    pGlyph->tu1, pGlyph->tv2 );
        D3DDevice::SetVertexData4f( D3DVSDE_VERTEX, right, top2,    pGlyph->tu1, pGlyph->tv1 );
        D3DDevice::SetVertexData4f( D3DVSDE_VERTEX, right, bottom2, pGlyph->tu2, pGlyph->tv1 );
    }
