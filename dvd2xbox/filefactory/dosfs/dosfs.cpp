...
{
  return g_memoryUnitManager.ReadFatDevice(unit, buffer, sector, count);
}

uint32_t DFS_WriteSector(uint8_t unit, uint8_t *buffer, uint32_t sector, uint32_t count)
{
  return g_memoryUnitManager.WriteFatDevice(unit, buffer, sector, count);
}
...
void DFS_Seek(PFILEINFO fileinfo, uint32_t offset, uint8_t *scratch)
{
	...
	// Don't allow the user to seek past the end of the file
	// Also return if we're seeking to where we already are
	if (offset > fileinfo->filelen || offset == fileinfo->pointer)
		return;
	...
	// very simple case
	if (div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).rem ==
	  div(fileinfo->pointer + offset, fileinfo->volinfo->secperclus * SECTOR_SIZE).rem) {
