...
int D2Xtexture::LoadTexturesFromXPR(const CStdString& strFileName)
{
	/*map<CStdString,LPDIRECT3DTEXTURE8>::iterator ibmp;
	ibmp = mapTexture.find(name.c_str());
	if(ibmp != mapTexture.end())
	...
	CXBPackedResource* pPackedResource = new CXBPackedResource();
	if ( SUCCEEDED( pPackedResource->Create( strFileName.c_str()) ) )
	{
		LPDIRECT3DTEXTURE8 pTexture;
		LPDIRECT3DTEXTURE8 m_pTexture;
		D3DSURFACE_DESC descSurface;
		...
		pPackedResource->GetResourceTags(&m_dwNumResourceTags,&m_pResourceTags);

		for( DWORD i=0; i<m_dwNumResourceTags; i++ )
		{

			pTexture = pPackedResource->GetTexture(m_pResourceTags[i].dwOffset);
			...
			if (pTexture)
			{
				if ( SUCCEEDED( pTexture->GetLevelDesc( 0, &descSurface ) ) )
				{
					...
					mapTexture.insert(pair<CStdString,LPDIRECT3DTEXTURE8>(m_pResourceTags[i].strName,m_pTexture));
					pSrcSurface->Release();
					pDestSurface->Release();
					pTexture->Release();
				}
